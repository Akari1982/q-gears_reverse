...
A0 = 1;
A1 = 271a;
8001D77C	jal    func1e218 [$8001e218]
...
loop1d7c8:	; 8001D7C8
    A0 = w[struct + 1c] + 854;
    func217c4();
8001D7DC	bne    v0, zero, loop1d7c8 [$8001d7c8]
...
A0 = w[V1 + 7d8];
A1 = 0;
8001D808	jal    func1c750 [$8001c750]
...
A2 = 00090000;
A3 = A1;
8001D824	jal    func1c8b0 [$8001c8b0]
...
else if( V1 == 2 )
{
    ...
    if( S1 != 0 )
    {
        ...
        A0 = w[struct + 20] + w[S0 + 0] + w[S0 + c] / 800;
        A1 = S1;
        A2 = w[S0 + 8];
        A3 = 0;
        func22c60(); // load sectors to memory
...
if( status == 1 ) // CdlDataReady
{
    ...
    {
        [800761d0 + 18] = w(sector);

        func2291c();

        // finish if num of sectors to read 0 or ???
        if( ( w[800761d0 + 4] == 0 ) || ( bu[800761d0 + 2] != 0 ) )
        {
            A0 = 0;
            system_psyq_cd_ready_callback();
...
if( dst != 0 )
{
    A0 = dst + (w[800761d0 + 10] / 4) * 4;
    A1 = 200;
    system_psyq_cd_get_sector();
}

[800761d0 + c] = w(w[800761d0 + c] + 800); // readed size
[800761d0 + 10] = w(w[800761d0 + 10] + 800); // cur dst offset
[800761d0 + 4] = w(w[800761d0 + 4] - 1); // num of sector to read

A0 = w[800761d0 + 14];

if( w[800761d0 + 10] >= w[S1 + A0 * 8 + 0] ) // compare with size
{
    [800761d0 + 10] = w(0);
    [800761d0 + 14] = w(A0 + 1);
...
[SP + 24] = w(800227c8); // cd_ready_callback - func227c8()
[SP + 28] = w(A1);
...
{
    A0 = A0 & 00ff;
    8002361C	jalr   w[800761d0 + 958] ra
}
////////////////////////////////
