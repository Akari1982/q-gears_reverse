...
.export TalkToObject, EnterLineupMenu, l1C_9C54
...
lut_ShopData:
  .INCBIN "bin/0E_shopdata_8300.bin"
...
lut_MenuText:
  .INCBIN "bin/0E_menutext_8500.bin"
...
EnterLineupMenu:
  ...
  @CopyLoop:
    ...
    LDA ch_OBailments           ; get char 0's OB ailments
    CLC
    ...
    LDA ch_OBailments+(1<<6)    ; do the same for char 1 / slot 1
    CLC
    ADC #$01
    AND #$02
    STA str_buf+(1*8)

    LDA ch_OBailments+(2<<6)    ; char 2 / slot 2
    CLC
    ADC #$01
    AND #$02
    STA str_buf+(2*8)

    LDA ch_OBailments+(3<<6)    ; char 3 / slot 3
    CLC
    ADC #$01
    AND #$02
    STA str_buf+(3*8)
...
l1C_9DF0:   LDX $65
    ...
    TAX
    BCC l1C_9E06
            LDA r1D_A011+$100, X        ; this is in the middle of code!  probably a bug!
    JMP l1C_9E09
l1C_9E06:   LDA r1D_A011, X
l1C_9E09:   STA $10
    LDA #$01
    STA $63
    LDA $62
...
CharName_DrawCursor:    ;; TODO -- COMMENTING
    LDA $64
    ASL A
    ASL A
    ASL A
...
EnterShop_Clinic:
    ...
    LDA #$00
    STA ch_OBailments, X       ; erase this character's ailments (curing his "death" ailment)
    LDA #$01
...
ClinicBuildNameString:
  ...
  @Loop:
    LDA ch_OBailments, X  ; get this char's OB ailments
...
DrawShopString:
    ...
    LDA r1C_8000, X    ; load the pointer from the shop string LUT
    STA text_ptr
    LDA r1C_8001, X
...
MenuFillPartyHP:
  ...
  @Loop:
    LDA ch_OBailments, X    ; get this character's ailment
...
MenuRecoverPartyMP:
  ...
  @Loop:
    LDA ch_OBailments, X   ; check OB ailments
    CMP #$01
...
MainMenuLoop:
  ...
  @NotItem:
    ...
    @MagicLoop:
      ...
      LDA ch_OBailments, X      ; get this character's OB ailments
      CMP #$01
...
UseMagic_CureFamily:
 ...
 CureFamily_Loop:
    ...
    LDA ch_OBailments, X    ; get target's OB ailments
    CMP #$01
...
UseItem_Heal:
  ...
  _UseItem_Heal_Loop:
    ...
    LDA ch_OBailments, X       ; check their OB ailments
    CMP #$01
...
CureOBAilment:
    ...
    TAX                   ; then stuff in X
    LDA ch_OBailments, X  ; get OB ailments
...
MenuRecoverHP:
    LDY ch_OBailments, X        ; get out of battle ailments for this character
    CPY #$01
