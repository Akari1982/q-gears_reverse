...
.export lut_RNG, l3F_F230, l3F_F22D, l3F_F22A, l3F_F21B, l3F_F227
.export l3F_F21E, l3F_F203, l3F_F206, CallMinimapDecompress
.export l3F_F218, l3F_F221, l3F_F224, l3F_F20F, l3F_F215, l3F_F212
.export l3F_F209, l3F_F20C



.import MinimapDecompress
.import EnterMinimap
.import r16_94E1, r16_94E0, l16_9A00, lut_BattleRates, lut_BattleFormations
...
.import TalkToObject, EnterLineupMenu, l1C_9C54
...
GameStart:
  ...
  @l3E_C031:
    ...
    LDA #BANK_INTRO
    JSR SwapPRG_L
    JSR EnterIntroStory         ; intro story
@l3E_C06F:
    LDA #BANK_TITLE
    JSR SwapPRG_L
...
DoOverworld:
   ...
   ; no JMP or RTS -- code flows seamlessly into EnterOverworldLoop
...
DoOWTransitions:
  ...
  @Battle:
    ...
    LDA btlformation
    JSR l3F_F200           ; start the battle!  DISCH TODO
    JMP EnterOW_PalCyc     ; then re-enter overworld
...
MapPoisonDamage:       ; first thing is to check if ANY characters are poisoned.
    LDA ch_OBailments  ; if nobody is poisoned, do nothing.
    CMP #$03                     ; get char 0's ailments... see if they=3 (poison)
    BEQ @DoIt                    ; if yes... "DoIt"

    LDA ch_OBailments + (1<<6)   ; and do the same with chars 1,2 and 3
    CMP #$03
    BEQ @DoIt
    LDA ch_OBailments + (2<<6)
    CMP #$03
    BEQ @DoIt
    LDA ch_OBailments + (3<<6)
    CMP #$03
    BEQ @DoIt
  ...
  @DmgLoop:
    LDA ch_OBailments, X  ; get this character's ailments
...
:     JSR ProcessSMInput    ; if none of those things -- process input, and continue
  ...
  @TeleOrBattle:
   ...
   ;;  Otherwise, here, this is a BATTLE
    ...
    LDA btlformation
    JSR l3F_F200            ; start the battle!
    BCC :+                  ;  see if this battle was the end game battle

    @VictoryLoop:
      JSR l3F_E89C            ; ????  DISCH -- TODO
...
@StatCode_Over01:
    ...
    BNE @StatCode_Over02
      ...
      LDA ch_OBailments, X  ; out-of-battle ailment ID
...
LoadBridgeSceneGFX:
    ...
    LDA #<lut_BridgeGFX      ; load a pointer to the bridge scene graphics (CHR first)
    STA tmp
    LDA #>lut_BridgeGFX
    STA tmp+1
    ...
    LDA #>(lut_BridgeGFX + $800)   ; reset the source pointer to the start of that NT data
    STA tmp+1
...
DrawOBSprite:
    ...
    LDA ch_OBailments, X  ; get out of battle ailment byte
...
:   STA draweq_stradd            ; this value will be later added to the weapon/armor ID to get
  ...
  @MainLoop:
    ...
    LDA r3F_ED72, Y              ; use Y to index a positioning LUT
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 ; F200 - enter battle
 ;   OUT:  C set = you beat the game

l3F_F200:   JMP l3F_F28D
l3F_F203:   JMP l3F_F23E
l3F_F206:   JMP l3F_F268
l3F_F209:   JMP l3F_F284
l3F_F20C:   JMP l3F_F620
l3F_F20F:   JMP l3F_F6B3
l3F_F212:   JMP l3F_F6C3
l3F_F215:   JMP l3F_F700
l3F_F218:   JMP l3F_F71C
l3F_F21B:   JMP l3F_F764
l3F_F21E:   JMP l3F_F4A1
l3F_F221:   JMP l3F_F89E
l3F_F224:   JMP l3F_F921
l3F_F227:   JMP l3F_FCE7
l3F_F22A:   JMP l3F_F440
l3F_F22D:   JMP l3F_FA59
l3F_F230:   JMP l3F_FCCF
l3F_F233:   LDA $97
    STA $2006
    LDA $96
    STA $2006
...
l3F_F28D:
  ...
  @PalLoop:                   ; copy the loaded palettes (backdrop, menu, sprites)
    ...
    LDA btl_formdata+10       ; use the formation data to get the ID of the palettes to load
    LDY #4                    ;   load the first one into the 2nd palette slot ($xxx4)
    JSR LoadBattlePalette
    LDA btl_formdata+11       ;   and the second one into the 3rd slot ($xxx8)
    LDY #8
    JSR LoadBattlePalette
  ...
  @ClearFFLoop:
    ...
    LDA #$0B                ; TODO -- still commenting (wtf is 6BB1?  last PRG bank for battle?)
    STA $6BB1
    JSR SwapPRG_L
    JMP l16_9A00
...
Battle_PPUOff:
    LDA #0
    STA a:soft2000     ; clear soft2000 and ?? soft2001??  (DISCH -- TODO unk_FE)
    STA a:unk_FE
    STA $2000          ; disable NMIs
...
lut_BtlAttrTbl:
  .BYTE $3F,$0F,$0F,$0F, $3F,$0F,$FF,$FF, $33,$00,$00,$00, $33,$00,$FF,$FF
  .BYTE $33,$00,$00,$00,$33,$00,$FF,$FF,$33,$00,$00,$00,$33,$00,$FF,$FF
  .BYTE $F3,$F0,$F0,$F0,$F3,$F0,$FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF
  .BYTE $FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F


l3F_F440:   LDA #$06
    STA $6D14
l3F_F445:   JSR l3F_F460
    JSR l3F_FCE7
    AND #$03
    STA $2005
    JSR l3F_FCE7
...
l3F_F5FD:   LDA ($8C), Y
    ...
    BNE l3F_F5FD
l3F_F607:   RTS
l3F_F608:   JSR l3F_F5FB
    JSR l3F_F59B
l3F_F60E:   JSR l3F_F5ED
    LDY #$00
    LDA ($8C), Y
    BEQ l3F_F607
    JSR l3F_F5FB
...
l3F_F6B3:   AND #$FF
    BEQ l3F_F6C2
    STA $6AA5
l3F_F6BA:   JSR l3F_F65A
    DEC $6AA5
    BNE l3F_F6BA
l3F_F6C2:   RTS
l3F_F6C3:   LDY #$05
l3F_F6C5:   LDA l3F_F9E3, Y
    STA $6A9D, Y
    DEY
    BNE l3F_F6C5
    JSR l3F_F690
...
l3F_FA81:   LDX #$00
    ...
    JSR l3F_FB96
    JMP l3F_FA94
l3F_FA91:   JSR l3F_FAA6
l3F_FA94:   JSR l3F_FCB2
    JMP l3F_FA81
l3F_FA9A:   LDA #$00
    LDY #$00
    STA ($92), Y
...
OnReset:
  ...
  @Loop: 
    ...
    JSR SwapPRG    ; swap to bank 6 (seems pointless to do this here)
