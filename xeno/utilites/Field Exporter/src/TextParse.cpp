...
TextParse( Logger* exp, File* file, const int offset )
{
    ...
    while( true )
    {
        ...
        else if( opcode == 0x0f )
        {
            ...
            if( sub_opcode == 0x00 )
            {
                exp->LogW( u"<SpecialWait wait=\"" + HexToU16String( file->GetU8( data_offset ), 2, '0' ) + u"\">" );
                data_offset += 1;
            }
            else if( sub_opcode == 0x01 )
            {
                exp->LogW( u"<SpecialLettersToRead number=\"" + HexToU16String( file->GetU8( data_offset ), 2, '0' ) + u"\">" );
                data_offset += 1;
            }
            else if( sub_opcode == 0x02 )
            {
                exp->LogW( u"<SpecialWaitAndClose wait=\"" + HexToU16String( file->GetU8( data_offset ), 2, '0' ) + u"\">" );
                data_offset += 1;
            }
            else if( sub_opcode == 0x05 )
            {
                exp->LogW( u"<SpecialCharacter char_id=\"" + HexToU16String( file->GetU8( data_offset ), 2, '0' ) + u"\">" );
                data_offset += 1;
            }
            else
            {
                exp->LogW( u"<special>" + HexToU16String( sub_opcode, 2, '0' ) + u"\n" );
                break;
            }
        }
        else if( opcode < 0xf8 )
