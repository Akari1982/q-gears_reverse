Actor = {}



Actor[ "0x00" ] = {
    on_start = function( self )
        -- 0xBC_ActorNoModelInit() -- 0x0016 0xbc
        -- 0xA0() -- 0x0017 0xa0
        -- 0x2A() -- 0x001e 0x2a
        return 0 -- 0x001f 0x00
    end,

    on_update = function( self )
        return 0 -- 0x0020 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0020 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0020 0x00
    end,

}



Actor[ "0x01" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=0 ) -- 0x0021 0x16
        opcodeFE0D_MessageSetFace( char_id=0 ) -- 0x0024 0xfe
        return 0 -- 0x0028 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0029 0x0c
        return 0 -- 0x002a 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x002a 0x00
    end,

    on_push = function( self )
        return 0 -- 0x002a 0x00
    end,

}



Actor[ "0x02" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=2 ) -- 0x002b 0x16
        opcodeFE0D_MessageSetFace( char_id=2 ) -- 0x002e 0xfe
        return 0 -- 0x0032 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0033 0x0c
        return 0 -- 0x0034 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0034 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0034 0x00
    end,

}



Actor[ "0x03" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=1 ) -- 0x0035 0x16
        opcodeFE0D_MessageSetFace( char_id=1 ) -- 0x0038 0xfe
        return 0 -- 0x003c 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x003d 0x0c
        return 0 -- 0x003e 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x003e 0x00
    end,

    on_push = function( self )
        return 0 -- 0x003e 0x00
    end,

}



Actor[ "0x04" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=3 ) -- 0x003f 0x16
        opcodeFE0D_MessageSetFace( char_id=3 ) -- 0x0042 0xfe
        return 0 -- 0x0046 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0047 0x0c
        return 0 -- 0x0048 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0048 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0048 0x00
    end,

}



Actor[ "0x05" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=4 ) -- 0x0049 0x16
        opcodeFE0D_MessageSetFace( char_id=4 ) -- 0x004c 0xfe
        return 0 -- 0x0050 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0051 0x0c
        return 0 -- 0x0052 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0052 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0052 0x00
    end,

}



Actor[ "0x06" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=5 ) -- 0x0053 0x16
        opcodeFE0D_MessageSetFace( char_id=5 ) -- 0x0056 0xfe
        return 0 -- 0x005a 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x005b 0x0c
        return 0 -- 0x005c 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x005c 0x00
    end,

    on_push = function( self )
        return 0 -- 0x005c 0x00
    end,

}



Actor[ "0x07" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=6 ) -- 0x005d 0x16
        opcodeFE0D_MessageSetFace( char_id=6 ) -- 0x0060 0xfe
        return 0 -- 0x0064 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0065 0x0c
        return 0 -- 0x0066 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0066 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0066 0x00
    end,

}



Actor[ "0x08" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=7 ) -- 0x0067 0x16
        opcodeFE0D_MessageSetFace( char_id=7 ) -- 0x006a 0xfe
        return 0 -- 0x006e 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x006f 0x0c
        return 0 -- 0x0070 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0070 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0070 0x00
    end,

}



Actor[ "0x09" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=8 ) -- 0x0071 0x16
        opcodeFE0D_MessageSetFace( char_id=8 ) -- 0x0074 0xfe
        return 0 -- 0x0078 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0079 0x0c
        return 0 -- 0x007a 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x007a 0x00
    end,

    on_push = function( self )
        return 0 -- 0x007a 0x00
    end,

}



Actor[ "0x0a" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=9 ) -- 0x007b 0x16
        opcodeFE0D_MessageSetFace( char_id=9 ) -- 0x007e 0xfe
        return 0 -- 0x0082 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x0083 0x0c
        return 0 -- 0x0084 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0084 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0084 0x00
    end,

}



Actor[ "0x0b" ] = {
    on_start = function( self )
        -- 0x16_ActorPCInit( char_id=10 ) -- 0x0085 0x16
        opcodeFE0D_MessageSetFace( char_id=10 ) -- 0x0088 0xfe
        return 0 -- 0x008c 0x00
    end,

    on_update = function( self )
        -- 0x0C() -- 0x008d 0x0c
        return 0 -- 0x008e 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x008e 0x00
    end,

    on_push = function( self )
        return 0 -- 0x008e 0x00
    end,

}



Actor[ "0x0c" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 4 ) -- 0x008f 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0x02b0, z=(vf40)0xfbee, flag=(flag)0xc0 ) -- 0x0092 0x19
        return 0 -- 0x0098 0x00
    end,

    on_update = function( self )
        -- 0x2A() -- 0x0099 0x2a
        opcode20_ActorSetFlags0( flags=13 ) -- 0x009a 0x20
        opcode2C_SpritePlayAnim( anim_id=0x00 ) -- 0x009d 0x2c
        -- 0xF6( ???=0x01 ) -- 0x009f 0xf6
        -- 0x21( ???=128 ) -- 0x00a1 0x21
        -- MISSING OPCODE 0xFE13
    end,

    on_talk = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)party_1 ) -- 0x00d7 0x6f
        -- MISSING OPCODE 0xFE13
    end,

    on_push = function( self )
        return 0 -- 0x00e5 0x00
    end,

}



Actor[ "0x0d" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 0 ) -- 0x00e6 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0x0311, z=(vf40)0xfcc4, flag=(flag)0xc0 ) -- 0x00e9 0x19
        return 0 -- 0x00ef 0x00
    end,

    on_update = function( self )
        -- 0x4A_ActorGoToPos( variable arguments based on 0x01800000 in script call ) -- 0x00f0 0x4a
        -- 0x4A_ActorGoToPos( variable arguments based on 0x01800000 in script call ) -- 0x00f6 0x4a
        -- 0x4A_ActorGoToPos( variable arguments based on 0x01800000 in script call ) -- 0x00fc 0x4a
        opcode69_ActorSetRotation( rot=0 ) -- 0x0102 0x69
        opcode26_Wait( time=10 ) -- 0x0105 0x26
        -- 0x2A() -- 0x0108 0x2a
        opcode20_ActorSetFlags0( flags=13 ) -- 0x0109 0x20
        opcode2C_SpritePlayAnim( anim_id=0x00 ) -- 0x010c 0x2c
        -- 0xF6( ???=0x01 ) -- 0x010e 0xf6
        -- 0x21( ???=128 ) -- 0x0110 0x21
        -- MISSING OPCODE 0xFE13
    end,

    on_talk = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)party_1 ) -- 0x0133 0x6f
        -- MISSING OPCODE 0x91
    end,

    on_push = function( self )
        return 0 -- 0x0175 0x00
    end,

}



Actor[ "0x0e" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 5 ) -- 0x0176 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0x017c, z=(vf40)0xfeec, flag=(flag)0xc0 ) -- 0x0179 0x19
        opcode69_ActorSetRotation( rot=7 ) -- 0x017f 0x69
        return 0 -- 0x0182 0x00
    end,

    on_update = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)0x0f ) -- 0x0183 0x6f
        return 0 -- 0x0185 0x00
    end,

    on_talk = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)party_1 ) -- 0x0186 0x6f
        opcode26_Wait( time=5 ) -- 0x0188 0x26
        -- 0x05_CallFunction( 0x0498 ) -- 0x018b 0x05
        opcodeD2_MessageShowDynamic( text_id=0x0007, flags=0 ) -- 0x018e 0xd2
        opcode9C_MessageSync() -- 0x0192 0x9c
        return 0 -- 0x0193 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0194 0x00
    end,

}



Actor[ "0x0f" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 3 ) -- 0x0195 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0x0133, z=(vf40)0xff33, flag=(flag)0xc0 ) -- 0x0198 0x19
        opcode69_ActorSetRotation( rot=5 ) -- 0x019e 0x69
        return 0 -- 0x01a1 0x00
    end,

    on_update = function( self )
        -- 0xC6() -- 0x01a2 0xc6
        opcode69_ActorSetRotation( rot=5 ) -- 0x01a3 0x69
        return 0 -- 0x01a6 0x00
    end,

    on_talk = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)party_1 ) -- 0x01a7 0x6f
        opcodeD2_MessageShowDynamic( text_id=0x0008, flags=0 ) -- 0x01a9 0xd2
        opcode9C_MessageSync() -- 0x01ad 0x9c
        return 0 -- 0x01ae 0x00
    end,

    on_push = function( self )
        return 0 -- 0x01af 0x00
    end,

}



Actor[ "0x10" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 2 ) -- 0x01b0 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0x036f, z=(vf40)0xff77, flag=(flag)0xc0 ) -- 0x01b3 0x19
        return 0 -- 0x01b9 0x00
    end,

    on_update = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)0x11 ) -- 0x01ba 0x6f
        return 0 -- 0x01bc 0x00
    end,

    on_talk = function( self )
        opcodeD2_MessageShowDynamic( text_id=0x0009, flags=0 ) -- 0x01bd 0xd2
        opcode9C_MessageSync() -- 0x01c1 0x9c
        return 0 -- 0x01c2 0x00
    end,

    on_push = function( self )
        return 0 -- 0x01c3 0x00
    end,

}



Actor[ "0x11" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 6 ) -- 0x01c4 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0x0334, z=(vf40)0xff3d, flag=(flag)0xc0 ) -- 0x01c7 0x19
        return 0 -- 0x01cd 0x00
    end,

    on_update = function( self )
        opcode6F_ActorRotateToActor( actor_id=(entity)0x10 ) -- 0x01ce 0x6f
        return 0 -- 0x01d0 0x00
    end,

    on_talk = function( self )
        opcodeD2_MessageShowDynamic( text_id=0x000a, flags=0 ) -- 0x01d1 0xd2
        opcode9C_MessageSync() -- 0x01d5 0x9c
        return 0 -- 0x01d6 0x00
    end,

    on_push = function( self )
        return 0 -- 0x01d6 0x00
    end,

}



Actor[ "0x12" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 7 ) -- 0x01d7 0x0b
        -- 0x19_ActorSetPosition( x=(vf80)0xfee0, z=(vf40)0x00c6, flag=(flag)0xc0 ) -- 0x01da 0x19
        return 0 -- 0x01e0 0x00
    end,

    on_update = function( self )
        opcode69_ActorSetRotation( rot=7 ) -- 0x01e1 0x69
        return 0 -- 0x01e4 0x00
    end,

    on_talk = function( self )
        opcodeD2_MessageShowDynamic( text_id=0x000b, flags=0 ) -- 0x01e5 0xd2
        opcode9C_MessageSync() -- 0x01e9 0x9c
        return 0 -- 0x01ea 0x00
    end,

    on_push = function( self )
        return 0 -- 0x01eb 0x00
    end,

}



Actor[ "0x13" ] = {
    on_start = function( self )
        -- 0xBC_ActorNoModelInit() -- 0x01ec 0xbc
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x01fd 0x00
    end,

    on_talk = function( self )
        -- 0xFE54() -- 0x01fe 0xfe
        -- 0x98_MapLoad( field_id=695, value=5 ) -- 0x0200 0x98
        return 0 -- 0x0205 0x00
    end,

    on_push = function( self )
        -- 0xFE54() -- 0x01fe 0xfe
        -- 0x98_MapLoad( field_id=695, value=5 ) -- 0x0200 0x98
        return 0 -- 0x0205 0x00
    end,

}



Actor[ "0x14" ] = {
    on_start = function( self )
        -- 0xBC_ActorNoModelInit() -- 0x0206 0xbc
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x021e 0x00
    end,

    on_talk = function( self )
        -- 0xFE54() -- 0x021f 0xfe
        opcode09_ActorCallScriptEW( actor_id=0x15, script=04, priority=03 ) -- 0x0221 0x09
        -- MISSING OPCODE 0xFE68
    end,

    on_push = function( self )
        return 0 -- 0x0231 0x00
    end,

}



Actor[ "0x15" ] = {
    on_start = function( self )
        -- 0xBC_ActorNoModelInit() -- 0x0232 0xbc
        -- MISSING OPCODE 0x46
    end,

    on_update = function( self )
        return 0 -- 0x0236 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0236 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0236 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0xc4
    end,

}



Actor[ "0x16" ] = {
    on_start = function( self )
        -- 0xBC_ActorNoModelInit() -- 0x023a 0xbc
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x024b 0x00
    end,

    on_talk = function( self )
        -- 0xFE54() -- 0x024c 0xfe
        -- 0x98_MapLoad( field_id=700, value=0 ) -- 0x024e 0x98
        return 0 -- 0x0253 0x00
    end,

    on_push = function( self )
        -- 0xFE54() -- 0x024c 0xfe
        -- 0x98_MapLoad( field_id=700, value=0 ) -- 0x024e 0x98
        return 0 -- 0x0253 0x00
    end,

}



