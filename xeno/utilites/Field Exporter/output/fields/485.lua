Entity = {}



Entity[ "0x00" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0047 0xbc
        -- MISSING OPCODE 0xb6
    end,

    on_update = function( self )
        return 0 -- 0x0069 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x006a 0x00
    end,

    on_push = function( self )
        return 0 -- 0x006b 0x00
    end,

}



Entity[ "0x01" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 0 ) -- 0x006c 0x16
        opcodeFE0D_SetAvatar( character_id=0 ) -- 0x006f 0xfe
        opcodeFE0D_SetAvatar( character_id=0 ) -- 0x0073 0xfe
        return 0 -- 0x0077 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x0083 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0084 0x00
    end,

}



Entity[ "0x02" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 1 ) -- 0x0085 0x16
        opcodeFE0D_SetAvatar( character_id=1 ) -- 0x0088 0xfe
        opcodeFE0D_SetAvatar( character_id=1 ) -- 0x008c 0xfe
        return 0 -- 0x0090 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x009c 0x00
    end,

    on_push = function( self )
        return 0 -- 0x009d 0x00
    end,

}



Entity[ "0x03" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 2 ) -- 0x009e 0x16
        opcodeFE0D_SetAvatar( character_id=2 ) -- 0x00a1 0xfe
        opcodeFE0D_SetAvatar( character_id=2 ) -- 0x00a5 0xfe
        return 0 -- 0x00a9 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x00b5 0x00
    end,

    on_push = function( self )
        return 0 -- 0x00b6 0x00
    end,

}



Entity[ "0x04" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 3 ) -- 0x00b7 0x16
        opcodeFE0D_SetAvatar( character_id=3 ) -- 0x00ba 0xfe
        opcodeFE0D_SetAvatar( character_id=3 ) -- 0x00be 0xfe
        return 0 -- 0x00c2 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x00ce 0x00
    end,

    on_push = function( self )
        return 0 -- 0x00cf 0x00
    end,

}



Entity[ "0x05" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 4 ) -- 0x00d0 0x16
        opcodeFE0D_SetAvatar( character_id=4 ) -- 0x00d3 0xfe
        opcodeFE0D_SetAvatar( character_id=4 ) -- 0x00d7 0xfe
        return 0 -- 0x00db 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x00e7 0x00
    end,

    on_push = function( self )
        return 0 -- 0x00e8 0x00
    end,

}



Entity[ "0x06" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 5 ) -- 0x00e9 0x16
        opcodeFE0D_SetAvatar( character_id=5 ) -- 0x00ec 0xfe
        opcodeFE0D_SetAvatar( character_id=5 ) -- 0x00f0 0xfe
        return 0 -- 0x00f4 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x0100 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0101 0x00
    end,

}



Entity[ "0x07" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 8 ) -- 0x0102 0x16
        opcodeFE0D_SetAvatar( character_id=8 ) -- 0x0105 0xfe
        opcodeFE0D_SetAvatar( character_id=8 ) -- 0x0109 0xfe
        return 0 -- 0x010d 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x0119 0x00
    end,

    on_push = function( self )
        return 0 -- 0x011a 0x00
    end,

}



Entity[ "0x08" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 7 ) -- 0x011b 0x16
        opcodeFE0D_SetAvatar( character_id=7 ) -- 0x011e 0xfe
        opcodeFE0D_SetAvatar( character_id=7 ) -- 0x0122 0xfe
        return 0 -- 0x0126 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x0132 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0133 0x00
    end,

}



Entity[ "0x09" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 6 ) -- 0x0134 0x16
        opcodeFE0D_SetAvatar( character_id=6 ) -- 0x0137 0xfe
        opcodeFE0D_SetAvatar( character_id=6 ) -- 0x013b 0xfe
        return 0 -- 0x013f 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x014b 0x00
    end,

    on_push = function( self )
        return 0 -- 0x014c 0x00
    end,

}



Entity[ "0x0a" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 14 ) -- 0x014d 0x16
        opcodeFE0D_SetAvatar( character_id=14 ) -- 0x0150 0xfe
        opcodeFE0D_SetAvatar( character_id=10 ) -- 0x0154 0xfe
        return 0 -- 0x0158 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x0164 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0165 0x00
    end,

}



Entity[ "0x0b" ] = {
    on_start = function( self )
        -- 0x16_EntityPCInit( 9 ) -- 0x0166 0x16
        opcodeFE0D_SetAvatar( character_id=9 ) -- 0x0169 0xfe
        opcodeFE0D_SetAvatar( character_id=2 ) -- 0x016d 0xfe
        return 0 -- 0x0171 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x017d 0x00
    end,

    on_push = function( self )
        return 0 -- 0x017e 0x00
    end,

}



Entity[ "0x0c" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 0 ) -- 0x017f 0x0b
        -- MISSING OPCODE 0x85
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xfb
    end,

    on_talk = function( self )
        return 0 -- 0x0279 0x00
    end,

    on_push = function( self )
        return 0 -- 0x027a 0x00
    end,

}



Entity[ "0x0d" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x027b 0xbc
        -- 0x19_SetPosition( x=(vf80)0xf51f, z=(vf40)0x0000, flag=(flag)0xc0 ) -- 0x027c 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x028c 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x028d 0xfe
        -- 0x35() -- 0x028f 0x35
        opcodeD2_MessageShow0( dialog_id=0x0000, ???=0x10 ) -- 0x0295 0xd2
        -- 0x9C() -- 0x0299 0x9c
        -- MISSING OPCODE 0x74
    end,

    on_push = function( self )
        return 0 -- 0x02a9 0x00
    end,

}



Entity[ "0x0e" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x02aa 0xbc
        return 0 -- 0x02ab 0x00
    end,

    on_update = function( self )
        return 0 -- 0x02ac 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x02ad 0x00
    end,

    on_push = function( self )
        return 0 -- 0x02ae 0x00
    end,

    script_0x04 = function( self )
        -- 0x35() -- 0x02af 0x35
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0410 ), value2=(s16)0x000b, condition="value1 == value2", jump_if_false=0x02c1 ) -- 0x02b5 0x02
        return 0 -- 0x02bd 0x00
    end,

}



Entity[ "0x0f" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x02d0 0xbc
        return 0 -- 0x02d1 0x00
    end,

    on_update = function( self )
        return 0 -- 0x02d2 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x02d3 0x00
    end,

    on_push = function( self )
        return 0 -- 0x02d4 0x00
    end,

    script_0x04 = function( self )
        -- 0x35() -- 0x02d5 0x35
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0412 ), value2=(s16)0x000b, condition="value1 == value2", jump_if_false=0x02e7 ) -- 0x02db 0x02
        return 0 -- 0x02e3 0x00
    end,

}



Entity[ "0x10" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x02f6 0xbc
        -- 0x19_SetPosition( x=(vf80)0xeb4f, z=(vf40)0x077b, flag=(flag)0xc0 ) -- 0x02f7 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x0307 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x0308 0xfe
        -- 0x35() -- 0x030a 0x35
        opcodeD2_MessageShow0( dialog_id=0x0001, ???=0x10 ) -- 0x0310 0xd2
        -- 0x9C() -- 0x0314 0x9c
        opcode09_EntityCallScriptEW( entity=0x11, script=04, priority=03 ) -- 0x0315 0x09
        -- 0x98_MapLoad( field_id=485, value=3 ) -- 0x0318 0x98
        return 0 -- 0x031d 0x00
    end,

    on_push = function( self )
        return 0 -- 0x031e 0x00
    end,

}



Entity[ "0x11" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x031f 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x0324 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0325 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0326 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x12" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x034b 0xbc
        -- 0x19_SetPosition( x=(vf80)0xeb59, z=(vf40)0xf886, flag=(flag)0xc0 ) -- 0x034c 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x035c 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x035d 0xfe
        -- 0x35() -- 0x035f 0x35
        opcodeD2_MessageShow0( dialog_id=0x0002, ???=0x10 ) -- 0x0365 0xd2
        -- 0x9C() -- 0x0369 0x9c
        opcode09_EntityCallScriptEW( entity=0x13, script=04, priority=03 ) -- 0x036a 0x09
        -- 0x98_MapLoad( field_id=485, value=9 ) -- 0x036d 0x98
        return 0 -- 0x0372 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0373 0x00
    end,

}



Entity[ "0x13" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0374 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x0379 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x037a 0x00
    end,

    on_push = function( self )
        return 0 -- 0x037b 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x14" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x03a0 0xbc
        -- 0x19_SetPosition( x=(vf80)0xfb1d, z=(vf40)0x0c9f, flag=(flag)0xc0 ) -- 0x03a1 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x03b1 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x03b2 0xfe
        -- 0x35() -- 0x03b4 0x35
        opcodeD2_MessageShow0( dialog_id=0x0003, ???=0x10 ) -- 0x03ba 0xd2
        -- 0x9C() -- 0x03be 0x9c
        opcode09_EntityCallScriptEW( entity=0x15, script=04, priority=03 ) -- 0x03bf 0x09
        -- 0x98_MapLoad( field_id=485, value=1 ) -- 0x03c2 0x98
        return 0 -- 0x03c7 0x00
    end,

    on_push = function( self )
        return 0 -- 0x03c8 0x00
    end,

}



Entity[ "0x15" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x03c9 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x03ce 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x03cf 0x00
    end,

    on_push = function( self )
        return 0 -- 0x03d0 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x16" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x03f5 0xbc
        -- 0x19_SetPosition( x=(vf80)0x0000, z=(vf40)0x0e00, flag=(flag)0xc0 ) -- 0x03f6 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x0406 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x0407 0xfe
        opcode09_EntityCallScriptEW( entity=0x17, script=04, priority=03 ) -- 0x0409 0x09
        -- 0x98_MapLoad( field_id=486, value=0 ) -- 0x040c 0x98
        return 0 -- 0x0411 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0412 0x00
    end,

}



Entity[ "0x17" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0413 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x0418 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0419 0x00
    end,

    on_push = function( self )
        return 0 -- 0x041a 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x18" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x043f 0xbc
        -- 0x19_SetPosition( x=(vf80)0x09dc, z=(vf40)0x09de, flag=(flag)0xc0 ) -- 0x0440 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x0450 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x0451 0xfe
        -- 0x35() -- 0x0453 0x35
        opcode09_EntityCallScriptEW( entity=0x19, script=04, priority=03 ) -- 0x0459 0x09
        -- 0x98_MapLoad( field_id=486, value=2 ) -- 0x045c 0x98
        return 0 -- 0x0461 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0462 0x00
    end,

}



Entity[ "0x19" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0463 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x0468 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0469 0x00
    end,

    on_push = function( self )
        return 0 -- 0x046a 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x1a" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x048f 0xbc
        -- 0x19_SetPosition( x=(vf80)0x0e0d, z=(vf40)0x0000, flag=(flag)0xc0 ) -- 0x0490 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x04a0 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x04a1 0xfe
        -- 0x35() -- 0x04a3 0x35
        opcode09_EntityCallScriptEW( entity=0x1b, script=04, priority=03 ) -- 0x04a9 0x09
        -- 0x98_MapLoad( field_id=486, value=4 ) -- 0x04ac 0x98
        return 0 -- 0x04b1 0x00
    end,

    on_push = function( self )
        return 0 -- 0x04b2 0x00
    end,

}



Entity[ "0x1b" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x04b3 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x04b8 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x04b9 0x00
    end,

    on_push = function( self )
        return 0 -- 0x04ba 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x1c" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x04df 0xbc
        -- 0x19_SetPosition( x=(vf80)0x09e4, z=(vf40)0xf60a, flag=(flag)0xc0 ) -- 0x04e0 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x04f0 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x04f1 0xfe
        -- 0x35() -- 0x04f3 0x35
        opcode09_EntityCallScriptEW( entity=0x1d, script=04, priority=03 ) -- 0x04f9 0x09
        -- 0x98_MapLoad( field_id=486, value=6 ) -- 0x04fc 0x98
        return 0 -- 0x0501 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0502 0x00
    end,

}



Entity[ "0x1d" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0503 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x0508 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0509 0x00
    end,

    on_push = function( self )
        return 0 -- 0x050a 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x1e" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x052f 0xbc
        -- 0x19_SetPosition( x=(vf80)0x0000, z=(vf40)0xf1e8, flag=(flag)0xc0 ) -- 0x0530 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x0540 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x0541 0xfe
        -- 0x35() -- 0x0543 0x35
        opcode09_EntityCallScriptEW( entity=0x1f, script=04, priority=03 ) -- 0x0549 0x09
        -- 0x98_MapLoad( field_id=486, value=8 ) -- 0x054c 0x98
        return 0 -- 0x0551 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0552 0x00
    end,

}



Entity[ "0x1f" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0553 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x0558 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0559 0x00
    end,

    on_push = function( self )
        return 0 -- 0x055a 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x20" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x057f 0xbc
        -- 0x19_SetPosition( x=(vf80)0xfb0b, z=(vf40)0xf359, flag=(flag)0xc0 ) -- 0x0580 0x19
        -- MISSING OPCODE 0xf8
    end,

    on_update = function( self )
        return 0 -- 0x0590 0x00
    end,

    on_talk = function( self )
        opcodeFE54() -- 0x0591 0xfe
        -- 0x35() -- 0x0593 0x35
        opcodeD2_MessageShow0( dialog_id=0x0004, ???=0x10 ) -- 0x0599 0xd2
        -- 0x9C() -- 0x059d 0x9c
        opcode09_EntityCallScriptEW( entity=0x21, script=04, priority=03 ) -- 0x059e 0x09
        -- 0x98_MapLoad( field_id=485, value=2 ) -- 0x05a1 0x98
        return 0 -- 0x05a6 0x00
    end,

    on_push = function( self )
        return 0 -- 0x05a7 0x00
    end,

}



Entity[ "0x21" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x05a8 0xbc
        -- MISSING OPCODE 0xFE07
    end,

    on_update = function( self )
        return 0 -- 0x05ad 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x05ae 0x00
    end,

    on_push = function( self )
        return 0 -- 0x05af 0x00
    end,

    script_0x04 = function( self )
        -- MISSING OPCODE 0x74
    end,

}



Entity[ "0x22" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x05d4 0xbc
        -- 0x2A() -- 0x05d5 0x2a
        return 0 -- 0x05d6 0x00
    end,

    on_update = function( self )
        -- MISSING OPCODE 0xa8
    end,

    on_talk = function( self )
        return 0 -- 0x05f1 0x00
    end,

    on_push = function( self )
        return 0 -- 0x05f2 0x00
    end,

}



Entity[ "0x23" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x05f3 0xbc
        -- 0x2A() -- 0x05f4 0x2a
        return 0 -- 0x05f5 0x00
    end,

    on_update = function( self )
        return 0 -- 0x05f6 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x05f7 0x00
    end,

    on_push = function( self )
        return 0 -- 0x05f8 0x00
    end,

    script_0x04 = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x061f ) -- 0x05f9 0x02
        -- MISSING OPCODE 0xFE84
    end,

    script_0x05 = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x0646 ) -- 0x0620 0x02
        -- MISSING OPCODE 0xFE84
    end,

    script_0x06 = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x066d ) -- 0x0647 0x02
        -- MISSING OPCODE 0xFE84
    end,

    script_0x07 = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x0694 ) -- 0x066e 0x02
        -- MISSING OPCODE 0xFE84
    end,

    script_0x08 = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x06bb ) -- 0x0695 0x02
        -- MISSING OPCODE 0xFE84
    end,

    script_0x09 = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x06e2 ) -- 0x06bc 0x02
        -- MISSING OPCODE 0xFE84
    end,

    script_0x0a = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0402 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x0709 ) -- 0x06e3 0x02
        -- MISSING OPCODE 0xFE84
    end,

}



Entity[ "0x24" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x070a 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x071e ) -- 0x070d 0x02
        -- 0x19_SetPosition( x=(vf80)0xf1aa, z=(vf40)0x0000, flag=(flag)0xc0 ) -- 0x0715 0x19
        -- 0x01_JumpTo( 0x0721 ) -- 0x071b 0x01
        -- 0x23() -- 0x071e 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0736 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0737 0x00
    end,

}



Entity[ "0x25" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0738 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x074c ) -- 0x073b 0x02
        -- 0x19_SetPosition( x=(vf80)0xf1aa, z=(vf40)0xffbc, flag=(flag)0xc0 ) -- 0x0743 0x19
        -- 0x01_JumpTo( 0x074f ) -- 0x0749 0x01
        -- 0x23() -- 0x074c 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0764 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0765 0x00
    end,

}



Entity[ "0x26" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0766 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x077a ) -- 0x0769 0x02
        -- 0x19_SetPosition( x=(vf80)0xf1aa, z=(vf40)0x0044, flag=(flag)0xc0 ) -- 0x0771 0x19
        -- 0x01_JumpTo( 0x077d ) -- 0x0777 0x01
        -- 0x23() -- 0x077a 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0792 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0793 0x00
    end,

}



Entity[ "0x27" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0794 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x07a8 ) -- 0x0797 0x02
        -- 0x19_SetPosition( x=(vf80)0xfd29, z=(vf40)0x0d2f, flag=(flag)0xc0 ) -- 0x079f 0x19
        -- 0x01_JumpTo( 0x07ab ) -- 0x07a5 0x01
        -- 0x23() -- 0x07a8 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x07c0 0x00
    end,

    on_push = function( self )
        return 0 -- 0x07c1 0x00
    end,

}



Entity[ "0x28" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x07c2 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x07d6 ) -- 0x07c5 0x02
        -- 0x19_SetPosition( x=(vf80)0xfcbb, z=(vf40)0x0ceb, flag=(flag)0xc0 ) -- 0x07cd 0x19
        -- 0x01_JumpTo( 0x07d9 ) -- 0x07d3 0x01
        -- 0x23() -- 0x07d6 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x07ee 0x00
    end,

    on_push = function( self )
        return 0 -- 0x07ef 0x00
    end,

}



Entity[ "0x29" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x07f0 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0804 ) -- 0x07f3 0x02
        -- 0x19_SetPosition( x=(vf80)0xfbf0, z=(vf40)0x0d04, flag=(flag)0xc0 ) -- 0x07fb 0x19
        -- 0x01_JumpTo( 0x0807 ) -- 0x0801 0x01
        -- 0x23() -- 0x0804 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x081c 0x00
    end,

    on_push = function( self )
        return 0 -- 0x081d 0x00
    end,

}



Entity[ "0x2a" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x081e 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0832 ) -- 0x0821 0x02
        -- 0x19_SetPosition( x=(vf80)0x052b, z=(vf40)0x0c7b, flag=(flag)0xc0 ) -- 0x0829 0x19
        -- 0x01_JumpTo( 0x0835 ) -- 0x082f 0x01
        -- 0x23() -- 0x0832 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x084a 0x00
    end,

    on_push = function( self )
        return 0 -- 0x084b 0x00
    end,

}



Entity[ "0x2b" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x084c 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0860 ) -- 0x084f 0x02
        -- 0x19_SetPosition( x=(vf80)0x049f, z=(vf40)0x0cb6, flag=(flag)0xc0 ) -- 0x0857 0x19
        -- 0x01_JumpTo( 0x0863 ) -- 0x085d 0x01
        -- 0x23() -- 0x0860 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0878 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0879 0x00
    end,

}



Entity[ "0x2c" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x087a 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x088e ) -- 0x087d 0x02
        -- 0x19_SetPosition( x=(vf80)0x05e0, z=(vf40)0x0bf5, flag=(flag)0xc0 ) -- 0x0885 0x19
        -- 0x01_JumpTo( 0x0891 ) -- 0x088b 0x01
        -- 0x23() -- 0x088e 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x08a6 0x00
    end,

    on_push = function( self )
        return 0 -- 0x08a7 0x00
    end,

}



Entity[ "0x2d" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x08a8 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x08bc ) -- 0x08ab 0x02
        -- 0x19_SetPosition( x=(vf80)0x0c5e, z=(vf40)0x059a, flag=(flag)0xc0 ) -- 0x08b3 0x19
        -- 0x01_JumpTo( 0x08bf ) -- 0x08b9 0x01
        -- 0x23() -- 0x08bc 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x08d4 0x00
    end,

    on_push = function( self )
        return 0 -- 0x08d5 0x00
    end,

}



Entity[ "0x2e" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x08d6 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x08ea ) -- 0x08d9 0x02
        -- 0x19_SetPosition( x=(vf80)0x0c5a, z=(vf40)0x0501, flag=(flag)0xc0 ) -- 0x08e1 0x19
        -- 0x01_JumpTo( 0x08ed ) -- 0x08e7 0x01
        -- 0x23() -- 0x08ea 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0902 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0903 0x00
    end,

}



Entity[ "0x2f" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0904 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0918 ) -- 0x0907 0x02
        -- 0x19_SetPosition( x=(vf80)0x0cca, z=(vf40)0x04b2, flag=(flag)0xc0 ) -- 0x090f 0x19
        -- 0x01_JumpTo( 0x091b ) -- 0x0915 0x01
        -- 0x23() -- 0x0918 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0930 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0931 0x00
    end,

}



Entity[ "0x30" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0932 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0946 ) -- 0x0935 0x02
        -- 0x19_SetPosition( x=(vf80)0x0cb2, z=(vf40)0xfb1b, flag=(flag)0xc0 ) -- 0x093d 0x19
        -- 0x01_JumpTo( 0x0949 ) -- 0x0943 0x01
        -- 0x23() -- 0x0946 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x095e 0x00
    end,

    on_push = function( self )
        return 0 -- 0x095f 0x00
    end,

}



Entity[ "0x31" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0960 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0974 ) -- 0x0963 0x02
        -- 0x19_SetPosition( x=(vf80)0x0c3d, z=(vf40)0xfacd, flag=(flag)0xc0 ) -- 0x096b 0x19
        -- 0x01_JumpTo( 0x0977 ) -- 0x0971 0x01
        -- 0x23() -- 0x0974 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x098c 0x00
    end,

    on_push = function( self )
        return 0 -- 0x098d 0x00
    end,

}



Entity[ "0x32" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x098e 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x09a2 ) -- 0x0991 0x02
        -- 0x19_SetPosition( x=(vf80)0x0c63, z=(vf40)0xfa56, flag=(flag)0xc0 ) -- 0x0999 0x19
        -- 0x01_JumpTo( 0x09a5 ) -- 0x099f 0x01
        -- 0x23() -- 0x09a2 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x09ba 0x00
    end,

    on_push = function( self )
        return 0 -- 0x09bb 0x00
    end,

}



Entity[ "0x33" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x09bc 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x09d0 ) -- 0x09bf 0x02
        -- 0x19_SetPosition( x=(vf80)0x059b, z=(vf40)0xf397, flag=(flag)0xc0 ) -- 0x09c7 0x19
        -- 0x01_JumpTo( 0x09d3 ) -- 0x09cd 0x01
        -- 0x23() -- 0x09d0 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x09e8 0x00
    end,

    on_push = function( self )
        return 0 -- 0x09e9 0x00
    end,

}



Entity[ "0x34" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x09ea 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x09fe ) -- 0x09ed 0x02
        -- 0x19_SetPosition( x=(vf80)0x052c, z=(vf40)0xf3ab, flag=(flag)0xc0 ) -- 0x09f5 0x19
        -- 0x01_JumpTo( 0x0a01 ) -- 0x09fb 0x01
        -- 0x23() -- 0x09fe 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0a16 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0a17 0x00
    end,

}



Entity[ "0x35" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0a18 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0a2c ) -- 0x0a1b 0x02
        -- 0x19_SetPosition( x=(vf80)0x04ba, z=(vf40)0xf334, flag=(flag)0xc0 ) -- 0x0a23 0x19
        -- 0x01_JumpTo( 0x0a2f ) -- 0x0a29 0x01
        -- 0x23() -- 0x0a2c 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0a44 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0a45 0x00
    end,

}



Entity[ "0x36" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0a46 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0a5a ) -- 0x0a49 0x02
        -- 0x19_SetPosition( x=(vf80)0xfd45, z=(vf40)0xf2ca, flag=(flag)0xc0 ) -- 0x0a51 0x19
        -- 0x01_JumpTo( 0x0a5d ) -- 0x0a57 0x01
        -- 0x23() -- 0x0a5a 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0a72 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0a73 0x00
    end,

}



Entity[ "0x37" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0a74 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0a88 ) -- 0x0a77 0x02
        -- 0x19_SetPosition( x=(vf80)0xfd76, z=(vf40)0xf271, flag=(flag)0xc0 ) -- 0x0a7f 0x19
        -- 0x01_JumpTo( 0x0a8b ) -- 0x0a85 0x01
        -- 0x23() -- 0x0a88 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0aa0 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0aa1 0x00
    end,

}



Entity[ "0x38" ] = {
    on_start = function( self )
        -- 0x0B_InitNPC( 1 ) -- 0x0aa2 0x0b
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003b, condition="value1 == value2", jump_if_false=0x0ab6 ) -- 0x0aa5 0x02
        -- 0x19_SetPosition( x=(vf80)0xfe44, z=(vf40)0xf2dc, flag=(flag)0xc0 ) -- 0x0aad 0x19
        -- 0x01_JumpTo( 0x0ab9 ) -- 0x0ab3 0x01
        -- 0x23() -- 0x0ab6 0x23
        -- MISSING OPCODE 0x27
    end,

    on_update = function( self )
        -- MISSING OPCODE 0x89
    end,

    on_talk = function( self )
        return 0 -- 0x0ace 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0acf 0x00
    end,

}



Entity[ "0x39" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0ad0 0xbc
        return 0 -- 0x0ad1 0x00
    end,

    on_update = function( self )
        return 0 -- 0x0ad2 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0ad3 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0ad3 0x00
    end,

}



Entity[ "0x3a" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0ad4 0xbc
        return 0 -- 0x0ad5 0x00
    end,

    on_update = function( self )
        return 0 -- 0x0ad6 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0ad7 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0ad7 0x00
    end,

}



Entity[ "0x3b" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0ad8 0xbc
        -- 0x84_ProgressLessEqualJumpTo( value=202, jump=0x0ae8 ) -- 0x0ad9 0x84
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x003a, condition="value1 == value2", jump_if_false=0x0ae8 ) -- 0x0ade 0x02
        -- MISSING OPCODE 0x28
    end,

    on_update = function( self )
        return 0 -- 0x0ae9 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0aea 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0aea 0x00
    end,

}



Entity[ "0x3c" ] = {
    on_start = function( self )
        -- 0xBC_EntityNoModelInit() -- 0x0aeb 0xbc
        return 0 -- 0x0aec 0x00
    end,

    on_update = function( self )
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0102 ), value2=(s16)0x0035, condition="value1 <= value2", jump_if_false=0x0b42 ) -- 0x0aed 0x02
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0002 ), value2=(s16)0x0000, condition="value1 == value2", jump_if_false=0x0b0b ) -- 0x0af5 0x02
        -- 0xF1() -- 0x0afd 0xf1
        -- 0x01_JumpTo( 0x0b42 ) -- 0x0b08 0x01
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0002 ), value2=(s16)0x0001, condition="value1 == value2", jump_if_false=0x0b21 ) -- 0x0b0b 0x02
        -- 0xF1() -- 0x0b13 0xf1
        -- 0x01_JumpTo( 0x0b42 ) -- 0x0b1e 0x01
        -- 0x02_ConditionalJumpTo( value1=GetVar( 0x0002 ), value2=(s16)0x0002, condition="value1 == value2", jump_if_false=0x0b37 ) -- 0x0b21 0x02
        -- 0xF1() -- 0x0b29 0xf1
        -- 0x01_JumpTo( 0x0b42 ) -- 0x0b34 0x01
        -- 0xF1() -- 0x0b37 0xf1
        return 0 -- 0x0b42 0x00
    end,

    on_talk = function( self )
        return 0 -- 0x0b43 0x00
    end,

    on_push = function( self )
        return 0 -- 0x0b43 0x00
    end,

}



