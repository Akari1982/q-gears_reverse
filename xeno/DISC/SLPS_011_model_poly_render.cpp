        ...
        if( ( SXY0 < w[8004f7a0] ) || ( SXY1 < w[8004f7a0] ) || ( SXY2P < w[8004f7a0] ) )
        {
            if( ( ( SXY0 & ffff ) < w[8004f79c] ) || ( ( SXY1 & ffff ) < w[8004f79c] ) || ( ( SXY2P & ffff ) < w[8004f79c] ) )
            {
                ...
                if( MAC0 > 0 )
                {
                    ...
                    RGB = w[normal + 0]; // Primary color
                    VXY0 = w[normal + 4]; // Normal vector
                    VZ0 = w[normal + 8]; // Normal vector
                    gte_NCCS(); // Normal color col. v0

                    [packet + 4] = w((T1 & ff000000) | (RGB2 & 00ffffff));
                    [rdata + depth * 4 + 0] = w(packet);
                    [packet] = w(w[rdata + depth * 4] | 04000000);
                }
            }
        }
    }
            ...
            if( ( ( SXY0 & ffff ) < w[8004f79c] ) || ( ( SXY1 & ffff ) < w[8004f79c] ) || ( ( SXY2P & ffff ) < w[8004f79c] ) )
            {
                ...
                if( MAC0 > 0 )
                {
                    ...
                    if( depth != 0 )
                    {
                        ...
                        [packet + 4] = w((bu[packet + 7] << 18) | (RGB2 & 00ffffff));

                        depth >>= w[8004f7a4];
                        T1 = w[rdata + depth * 4];
                        [rdata + depth * 4] = w(packet);
                        [packet] = w(T1 | 07000000);
                    }
                }
            }
        }
