...
u8* g_reverb_workarea_cur;      // 0x8004a69c
...
u32 g_spu_base = 0x1f801c00;    // 0x8004aaf4
u32 g_spu_shift;                // 0x8004ab1c
u32 g_spu_delayed_set;          // 0x8004aaec

u32 g_spu_mem_table;            // 0x8004ab58
u32 g_spu_mem_alloc_n;          // 0x8004ab54 number of times spu memory is allocated
u32 g_spu_mem_alloc_max;        // 0x8004ab50 max number of times spu memory is allocated

u32 g_transfer_mode1;           // 0x8004a690
u32 g_transfer_mode2;           // 0x8004ab10
u32 g_transfer_callback;        // 0x8004ab2c
u32 g_spu_irq_callback;         // 0x8004ab30

u32 g_spu_key;                  // 0x8004a68c
...
void func362b8( S0 )
{
    ...
    g_reverb_on = SPU_OFF;
    [0x8004a698] = w(0);
    g_reverb_workarea_cur = w[0x8004ab5c + 0x0 * 0x4]; // set default work area start address for reverb mode 0.
    ...
    g_spu_mem_alloc_max = 0;
    g_spu_mem_alloc_n = 0;
    g_spu_mem_table = 0;

    g_spu_key = 0;

    g_transfer_mode1 = SPU_TRANSFER_BY_DMA;
    g_transfer_mode2 = SPU_TRANSFER_BY_DMA;
...
void func3642c( without_data )
{
    ...
    [g_spu_base + 0x180] = h(0); // Mainvolume left
    [g_spu_base + 0x182] = h(0); // Mainvolume right
    [g_spu_base + 0x1aa] = h(0); // SPU Control Register
    ...
    if( hu[g_spu_base + 0x1ae] & 0x07ff ) // SPU Status Register
    {
        V1 = 1;
        loop364b8:	; 800364B8
            if( V1 >= 0xf01 )
    ...
    [g_spu_base + 0x1ac] = h(0x0004); // Sound RAM Data Transfer Control (should be 0004h)
    [g_spu_base + 0x184] = h(0x0); // Reverb Output Volume Left
    [g_spu_base + 0x186] = h(0x0); // Reverb Output Volume Right
    [g_spu_base + 0x18c] = h(0xffff); // Key OFF lower
    [g_spu_base + 0x18e] = h(0xffff); // Key OFF upper
    [g_spu_base + 0x198] = h(0x0); // Reverb mode aka Echo On lower
    [g_spu_base + 0x19a] = h(0x0); // Reverb mode aka Echo On upper
    ...
    if( without_data == 0 )
    {
        ...
        [g_spu_base + 0x190] = h(0); // Pitch Modulation Enable Flags lower
        [g_spu_base + 0x192] = h(0); // Pitch Modulation Enable Flags upper
        [g_spu_base + 0x194] = h(0); // Noise mode enable lower
        [g_spu_base + 0x196] = h(0); // Noise mode enable upper
        [g_spu_base + 0x1b0] = h(0); // CD Audio Input Volume Left
        [g_spu_base + 0x1b2] = h(0); // CD Audio Input Volume Right
        [g_spu_base + 0x1b4] = h(0); // External Audio Input Volume Left
        [g_spu_base + 0x1b6] = h(0); // External Audio Input Volume Right
        ...
        {
            [g_spu_base + i * 0x10 + 0x0] = h(0); // Volume Left
            [g_spu_base + i * 0x10 + 0x2] = h(0); // Volume Right
            [g_spu_base + i * 0x10 + 0x4] = h(0x3fff); // Reverb Output Volume Left
            [g_spu_base + i * 0x10 + 0x6] = h(0x0200); // Reverb Output Volume Right
            [g_spu_base + i * 0x10 + 0x8] = h(0); // Key ON lower
            [g_spu_base + i * 0x10 + 0xa] = h(0); // Key ON upper
        }

        [g_spu_base + 0x188] = h(0xffff); // Key ON lower
        [g_spu_base + 0x18a] = h(0x00ff); // Key ON upper
        ...
        [g_spu_base + 0x18c] = h(0xffff); // Key OFF upper
        [g_spu_base + 0x18e] = h(0x00ff); // Key OFF lower
    ...
    [g_spu_base + 0x1aa] = h(0xc000); // SPU Control Register

    [0x8004ab28] = w(1);
    g_transfer_callback = 0;
    g_spu_irq_callback = 0;
...
// set to 0x0200
[g_spu_base + 1a6] = h(hu[0x8004ab0c]); // Sound RAM Data Transfer Address

S3 = hu[g_spu_base + 1ae] & 07ff; // SPU Status Register
...
if( size != 0 )
{
    loop36708:	; 80036708
        ...
        if( one_time_size > 0 )
        {
            V1 = 0;
            loop36724:	; 80036724
                [g_spu_base + 1a8] = h(hu[address + V1]); // Sound RAM Data Transfer Fifo (max 32 halfwords)
                V1 = V1 + 2;
                V0 = V1 < one_time_size;
        ...
        [g_spu_base + 1aa] = h((hu[g_spu_base + 1aa] & ffcf) | 0010); // SPU Control Register

        system_sound_wait_sync();

        if( hu[g_spu_base + 1ae] & 0400 ) // SPU Status Register
        {
            V1 = 1;
            loop3678c:	; 8003678C
                if( V1 >= f01 )
                ...
                V1 = V1 + 0001;
                V0 = hu[g_spu_base + 1ae] & 0400; // SPU Status Register
...
[g_spu_base + 1aa] = h(hu[g_spu_base + 1aa] & ffcf); // SPU Control Register

if( ( hu[g_spu_base + 1ae] & 07ff ) != S3 )
{
    V1 = 1;
    loop36820:	; 80036820
        if( V1 >= f01 )
...
[g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xffcf);

V1 = 0;
while( hu[g_spu_base + 0x1aa] & 0x30 )
{
    V1 += 0x1;
...
if( g_transfer_callback != 0 )
{
    g_transfer_callback();
}
else
{
    system_bios_deliver_event( 0xf0000009, 0x20 );
}
////////////////////////////////
...
{
    if( g_transfer_mode2 == SPU_TRANSFER_BY_DMA )
    {
        func36a18( 0x2, hu[0x8004ab0c] << g_spu_shift ); // set address in spu to write to 0x1f801da6
...
{
    func36a18( 2, hu[0x8004ab0c] << g_spu_shift );
...
void func36d98( A0, A1, A2 )
{
    if( A2 == 0 )
    {
        [g_spu_base + A0 * 0x2 + 0x0] = h(A1);
    }
    else
    {
        [g_spu_base + A0 * 0x2 + 0x0] = h(A1 >> g_spu_shift);
    }
}
...
u32 func36de0( A2, A1 )
{
    ...
    {
        [g_spu_base + A2 * 2] = h(A1 >> g_spu_shift);
...
void system_psyq_spu_init_malloc( long num, char* top )
{
    ...
    [top + 0x4] = w((0x10000 << g_spu_shift) - 0x1010); // set size to full memory

    g_spu_mem_alloc_max = num;
    g_spu_mem_alloc_n = 0;
    g_spu_mem_table = top;
}
...
{
    flag_b = w[g_spu_mem_table + block * 0x8 + 0x0];
    addr_b = flag_b & 0x0fffffff;
    size_b = w[g_spu_mem_table + block * 0x8 + 0x4];

    addr_ofs = addr - addr_b;

    V1 = ( w[0x8004a698] == 0 ) ? 0 : (0x10000 - g_reverb_workarea_cur) << g_spu_shift;
    ...
    if( flag_b & 0x40000000 )
    {
        if( addr_ofs > 0 )
        {
            id1 = g_spu_mem_alloc_n;
            id2 = id1 + 1;

            if( (g_spu_mem_alloc_max - 2) < id1 ) return -1;
            g_spu_mem_alloc_n = id2;

            [g_spu_mem_table + id1 * 0x8 + 0x0] = w((w[g_spu_mem_table + id1 * 0x8 + 0x0] & 0x0fffffff) | 0x80000000); // set as start block
            [g_spu_mem_table + id1 * 0x8 + 0x4] = w(addr_ofs);
            [g_spu_mem_table + id2 * 0x8 + 0x0] = w(addr);
            [g_spu_mem_table + id2 * 0x8 + 0x4] = w(size);
        }
        else
        {
            id1 = g_spu_mem_alloc_n;

            if( (g_spu_mem_alloc_max - 1) < id1 ) return -1;

            [g_spu_mem_table + id1 * 0x8 + 0x0] = w(w[g_spu_mem_table + id1 * 8 + 0x0] & 0x0fffffff);
            [g_spu_mem_table + id1 * 0x8 + 0x4] = w(size);
        }

        id = g_spu_mem_alloc_n + 1;
        g_spu_mem_alloc_n = id;

        [g_spu_mem_table + id * 0x8 + 0x0] = w((addr_b + addr_ofs + size) | 0x40000000); // set as end block
        [g_spu_mem_table + id * 0x8 + 0x4] = w(size_b - addr_ofs - size);
    }
    else
    {
        if( addr_ofs > 0 )
        {
            V0 = ( size_left == size ) ? g_spu_mem_alloc_max - 2 : g_spu_mem_alloc_max - 1;
            if( V0 < g_spu_mem_alloc_n ) return -1;

            id1 = g_spu_mem_alloc_n;
            id2 = id1 + 1;
            g_spu_mem_alloc_n = id1 + 1;

            [g_spu_mem_table + block * 0x8 + 0x4] = w(addr_ofs);

            addr_1 = w[g_spu_mem_table + id1 * 0x8 + 0x0];
            size_1 = w[g_spu_mem_table + id1 * 0x8 + 0x4];

            [g_spu_mem_table + id1 * 0x8 + 0x0] = w(addr);
            [g_spu_mem_table + id1 * 0x8 + 0x4] = w(size);
            [g_spu_mem_table + id2 * 0x8 + 0x0] = w(addr_1);
            [g_spu_mem_table + id2 * 0x8 + 0x4] = w(size_1);

            if( size_left >= size )
            {
                id3 = id1 + 2;
                g_spu_mem_alloc_n = id1 + 0x2;

                addr_2 = w[g_spu_mem_table + id2 * 0x8 + 0x0];
                size_2 = w[g_spu_mem_table + id2 * 0x8 + 0x4];

                [g_spu_mem_table + id2 * 0x8 + 0x0] = w((addr_b + addr_ofs + size) | 0x80000000); // set as start block
                [g_spu_mem_table + id2 * 0x8 + 0x4] = w(size_left - size);
                [g_spu_mem_table + id3 * 0x8 + 0x0] = w(addr_2);
                [g_spu_mem_table + id3 * 0x8 + 0x4] = w(size_2);
            }
        }
        else
        {
            if( size < size_left )
            {
                if( (g_spu_mem_alloc_max - 1) < g_spu_mem_alloc_n ) return -1;
            }

            [g_spu_mem_table + block * 0x8 + 0x0] = w(w[g_spu_mem_table + block * 0x8 + 0x0] & 0x0fffffff);
            [g_spu_mem_table + block * 0x8 + 0x4] = w(size);
            ...
            if( size < size_left )
            {
                ...
                [g_spu_mem_table + id1 * 0x8 + 0x0] = w((addr_b + size) | 0x80000000); // set as start block
                [g_spu_mem_table + id1 * 0x8 + 0x4] = w(size_left - size);
                [g_spu_mem_table + id2 * 0x8 + 0x0] = w(addr_1);
                [g_spu_mem_table + id2 * 0x8 + 0x4] = w(size_1);
            }
        }
...
{
    S2 = ( w[0x8004a698] == 0 ) ? 0 : (0x10000 - g_reverb_workarea_cur) << g_spu_shift;

    // align size and addr
    if( size & ~w[0x8004ab24] ) size += w[0x8004ab24];
    size = (size >> g_spu_shift) << g_spu_shift;
    if( addr & ~w[0x8004ab24] ) addr += w[0x8004ab24];
    addr = (addr >> g_spu_shift) << g_spu_shift;

    if( addr < 0x1010 ) return -1;

    if( (addr + size) > ((0x10000 << g_spu_shift) - S2) ) return -1;

    if( w[g_spu_mem_table + 0 * 0x8 + 0x0] & 0x40000000 ) // is end block
    {
        mem_end_size = (0x1010 + w[g_spu_mem_table + 0 * 0x8 + 0x4]) - (addr + size);

        if( (addr - 0x1010) > 0 ) // if addr is after reserved area
        {
            if( (g_spu_mem_alloc_max - 1) < g_spu_mem_alloc_n ) return -1; // max num of allocation exceed

            [g_spu_mem_table + 0 * 0x8 + 0x0] = w(0x80001010);
            [g_spu_mem_table + 0 * 0x8 + 0x4] = w(addr - 0x1010);
            [g_spu_mem_table + 1 * 0x8 + 0x0] = w(addr);
            [g_spu_mem_table + 1 * 0x8 + 0x4] = w(size);
            [g_spu_mem_table + 2 * 0x8 + 0x0] = w((addr + size) | 0x40000000);
            [g_spu_mem_table + 2 * 0x8 + 0x4] = w(mem_end_size);

            g_spu_mem_alloc_n = 2; // double allocation
        }
        else // if addr is inside reserved area
        {
            if( (g_spu_mem_alloc_max - 1) < g_spu_mem_alloc_n ) return -1; // max num of allocation exceed

            [g_spu_mem_table + 0 * 0x8 + 0x0] = w(addr);
            [g_spu_mem_table + 0 * 0x8 + 0x4] = w(size);
            [g_spu_mem_table + 1 * 0x8 + 0x0] = w((addr + size) | 0x40000000);
            [g_spu_mem_table + 1 * 0x8 + 0x4] = w(mem_end_size);

            g_spu_mem_alloc_n = 1; // single allocation
        }
    }
    else
    {
        ...
        while( true )
        {
            A1 = w[g_spu_mem_table + i * 8 + 0x0];
            addr_i = A1 & 0x0fffffff;
            size_i = w[g_spu_mem_table + i * 8 + 0x4];
            ...
            if( A1 & 0x80000000 )
            {
                ...
                {
                    if( (addr < (addr_i + size_i)) || (w[g_spu_mem_table + (i + 1) * 8 + 0x0] & 0x40000000) )
                    {
...
{
    // concat free blocks
    for( int i = 0; i <= g_spu_mem_alloc_n; ++i )
    {
        if( w[g_spu_mem_table + i * 0x8 + 0x0] & 0x80000000 )
        {
            int j = i + 1;
            while( w[g_spu_mem_table + j * 0x8 + 0x0] == 0x2fffffff ) j += 1; // skip empty block

            if( w[g_spu_mem_table + j * 0x8 + 0x0] & 0x80000000 )
            {
                if( (w[g_spu_mem_table + j * 0x8 + 0x0] & 0x0fffffff) == ((w[g_spu_mem_table + i * 0x8 + 0x0] & 0x0fffffff) + w[g_spu_mem_table + i * 0x8 + 0x4]) )
                {
                    [g_spu_mem_table + j * 0x8 + 0x0] = w(0x2fffffff);
                    [g_spu_mem_table + i * 0x8 + 0x4] = w(w[g_spu_mem_table + i * 0x8 + 0x4] + w[g_spu_mem_table + j * 0x8 + 0x4]);
    ...
    for( int i = 0; i <= g_spu_mem_alloc_n; ++i )
    {
        if( w[g_spu_mem_table + i * 0x8 + 0x4] == 0 ) [g_spu_mem_table + i * 0x8 + 0x0] = w(0x2fffffff);
    }
    ...
    for( int i = 0; i <= g_spu_mem_alloc_n; ++i )
    {
        if( w[g_spu_mem_table + i * 0x8 + 0x0] & 0x40000000 ) break; // finish if end block

        for( int j = (i + 1); j <= g_spu_mem_alloc_n; ++j )
        {
            if( w[g_spu_mem_table + j * 0x8 + 0x0] & 0x40000000 ) break; // finish if end block

            addr_i = w[g_spu_mem_table + i * 0x8 + 0x0];
            addr_j = w[g_spu_mem_table + j * 0x8 + 0x0];
            size_i = w[g_spu_mem_table + i * 0x8 + 0x4];
            size_j = w[g_spu_mem_table + j * 0x8 + 0x4];

            if( (addr_j & 0x0fffffff) < (addr_i & 0x0fffffff) )
            {
                [g_spu_mem_table + i * 0x8 + 0x0] = w(addr_j);
                [g_spu_mem_table + i * 0x8 + 0x4] = w(size_j);
                [g_spu_mem_table + j * 0x8 + 0x0] = w(addr_i);
                [g_spu_mem_table + j * 0x8 + 0x4] = w(size_i);
            }
        }
    }

    // remove empty blocks
    end_id = g_spu_mem_alloc_n;
    for( int i = 0; i <= end_id; ++i )
    {
        V1 = w[g_spu_mem_table + i * 0x8 + 0x0];

        if( V1 & 0x40000000 ) break; // finish if end block

        if( V1 == 0x2fffffff )
        {
            [g_spu_mem_table + i * 0x8 + 0x0] = w(w[g_spu_mem_table + end_id * 8 + 0x0]);
            [g_spu_mem_table + i * 0x8 + 0x4] = w(w[g_spu_mem_table + end_id * 8 + 0x4]);
    ...
    // concat end blocks
    for( int i = (g_spu_mem_alloc_n - 1); i >= 0; --i )
    {
        if( ( w[g_spu_mem_table + i * 0x8 + 0x0] & 0x80000000 ) == 0 ) break; // finish if free block

        [g_spu_mem_table + i * 0x8 + 0x0] = w((w[g_spu_mem_table + i * 0x8 + 0x0] & 0x0fffffff) | 0x40000000);

        V1 = g_spu_mem_alloc_n;
        g_spu_mem_alloc_n = i;

        [g_spu_mem_table + i * 0x8 + 0x4] = w(w[g_spu_mem_table + i * 0x8 + 0x4] + w[g_spu_mem_table + V1 * 0x8 + 0x4]);
    }
}
...
{
    if( g_spu_delayed_set & 0x1 )
    {
        T2 = hu[0x80077da0 + A2 * 2] | (hu[0x80077da0 + A3 * 2] & 0xff) << 0x10;
    }
    else
    {
        T2 = hu[g_spu_base + A2 * 2] | (hu[g_spu_base + A3 * 2] & 0xff) << 0x10;
    }
    ...
    if( on_off != 0 )
    {
        if( on_off == 1 )
        {
            ...
            {
                [g_spu_base + A2 * 2] = h(hu[g_spu_base + A2 * 2] | voice_bit);
                [g_spu_base + A3 * 2] = h(hu[g_spu_base + A3 * 2] | (voice_bit >> 0x10) & 0xff);
            }
    ...
    else
    {
        ...
        {
            [g_spu_base + A2 * 2] = h(hu[g_spu_base + A2 * 2] & ~voice_bit);
            [g_spu_base + A3 * 2] = h(hu[g_spu_base + A3 * 2] & (~((voice_bit >> 10) & 0xff)));
        }
...
long system_psyq_spu_set_noise_clock( long n_clock )
{
    ...
    [g_spu_base + 0x1aa] = h((hu[g_spu_base + 0x1aa] & 0xc0ff) | ((n_clock & 0x3f) << 0x8));
...
u_long system_psyq_spu_read( u_char* addr, u_long size )
{
    ...
    if( g_transfer_callback == 0 ) [0x8004ab28] = w(0);
...
long system_psyq_spu_set_reverb( long on_off )
{
    ...
    {
        g_reverb_on = 0;
        [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xff7f); // Reverb Master Disable
    }
    else if( on_off == 1 )
    {
        if( w[0x8004a698] != 1 )
        ...
        g_reverb_on = on_off;
        [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0080); // Reverb Master Enable
    }
...
long system_psyq_spu_set_reverb_mode_param( SpuReverbAttr* attr )
{
    ...
    if( (mask == 0) || (mask & SPU_REV_MODE) )
    {
        ...
        g_reverb_workarea_cur = w[0x8004ab5c + mode * 4];
    ...
    if( loaded_mode != 0 )
    {
        rev_enabled = (hu[g_spu_base + 0x1aa] >> 0x7) & 0x1; // Reverb Master Enable    (0=Disabled, 1=Enabled)

        if( rev_enabled != 0 )
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xff7f); // disable reverb
        }
        [g_spu_base + 0x184] = h(0);
        [g_spu_base + 0x186] = h(0);
        g_reverb_depth_left = 0;
        g_reverb_depth_right = 0;
    }
    else
    {
        ...
        {
            [g_spu_base + 0x184] = h(attr->depth.left);
            g_reverb_depth_left = attr->depth.left;
        }

        if( (mask == 0) || (mask & SPU_REV_DEPTHR) )
        {
            [g_spu_base + 0x186] = h(attr->depth.right);
    ...
    {
        system_spu_set_reverb_reg( SP + 0x10 );
    }

    if( clear_wa == true )
    {
        system_spu_reverb_clear_workarea( g_reverb_mode );
    }

    if( loaded_mode != 0 )
    {
        func36d98( 0xd1, g_reverb_workarea_cur, 0 ); // 1a2 Sound RAM Reverb Work Area Start Address

        if( rev_enabled != 0 )
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0080); // reverb enable
        }
    }
...
void system_spu_set_reverb_reg( A0 )
{
    u32 mask = w[A0 + 0x0];

    if( (mask == 0) || (mask & 0x00000001) ) [g_spu_base + 0x1c0] = h(hu[A0 + 0x04]);
    if( (mask == 0) || (mask & 0x00000002) ) [g_spu_base + 0x1c2] = h(hu[A0 + 0x06]);
    if( (mask == 0) || (mask & 0x00000004) ) [g_spu_base + 0x1c4] = h(hu[A0 + 0x08]);
    if( (mask == 0) || (mask & 0x00000008) ) [g_spu_base + 0x1c6] = h(hu[A0 + 0x0a]);
    if( (mask == 0) || (mask & 0x00000010) ) [g_spu_base + 0x1c8] = h(hu[A0 + 0x0c]);
    if( (mask == 0) || (mask & 0x00000020) ) [g_spu_base + 0x1ca] = h(hu[A0 + 0x0e]);
    if( (mask == 0) || (mask & 0x00000040) ) [g_spu_base + 0x1cc] = h(hu[A0 + 0x10]);
    if( (mask == 0) || (mask & 0x00000080) ) [g_spu_base + 0x1ce] = h(hu[A0 + 0x12]);
    if( (mask == 0) || (mask & 0x00000100) ) [g_spu_base + 0x1d0] = h(hu[A0 + 0x14]);
    if( (mask == 0) || (mask & 0x00000200) ) [g_spu_base + 0x1d2] = h(hu[A0 + 0x16]);
    if( (mask == 0) || (mask & 0x00000400) ) [g_spu_base + 0x1d4] = h(hu[A0 + 0x18]);
    if( (mask == 0) || (mask & 0x00000800) ) [g_spu_base + 0x1d6] = h(hu[A0 + 0x1a]);
    if( (mask == 0) || (mask & 0x00001000) ) [g_spu_base + 0x1d8] = h(hu[A0 + 0x1c]);
    if( (mask == 0) || (mask & 0x00002000) ) [g_spu_base + 0x1da] = h(hu[A0 + 0x1e]);
    if( (mask == 0) || (mask & 0x00004000) ) [g_spu_base + 0x1dc] = h(hu[A0 + 0x20]);
    if( (mask == 0) || (mask & 0x00008000) ) [g_spu_base + 0x1de] = h(hu[A0 + 0x22]);
    if( (mask == 0) || (mask & 0x00010000) ) [g_spu_base + 0x1e0] = h(hu[A0 + 0x24]);
    if( (mask == 0) || (mask & 0x00020000) ) [g_spu_base + 0x1e2] = h(hu[A0 + 0x26]);
    if( (mask == 0) || (mask & 0x00040000) ) [g_spu_base + 0x1e4] = h(hu[A0 + 0x28]);
    if( (mask == 0) || (mask & 0x00080000) ) [g_spu_base + 0x1e6] = h(hu[A0 + 0x2a]);
    if( (mask == 0) || (mask & 0x00100000) ) [g_spu_base + 0x1e8] = h(hu[A0 + 0x2c]);
    if( (mask == 0) || (mask & 0x00200000) ) [g_spu_base + 0x1ea] = h(hu[A0 + 0x2e]);
    if( (mask == 0) || (mask & 0x00400000) ) [g_spu_base + 0x1ec] = h(hu[A0 + 0x30]);
    if( (mask == 0) || (mask & 0x00800000) ) [g_spu_base + 0x1ee] = h(hu[A0 + 0x32]);
    if( (mask == 0) || (mask & 0x01000000) ) [g_spu_base + 0x1f0] = h(hu[A0 + 0x34]);
    if( (mask == 0) || (mask & 0x02000000) ) [g_spu_base + 0x1f2] = h(hu[A0 + 0x36]);
    if( (mask == 0) || (mask & 0x04000000) ) [g_spu_base + 0x1f4] = h(hu[A0 + 0x38]);
    if( (mask == 0) || (mask & 0x08000000) ) [g_spu_base + 0x1f6] = h(hu[A0 + 0x3a]);
    if( (mask == 0) || (mask & 0x10000000) ) [g_spu_base + 0x1f8] = h(hu[A0 + 0x3c]);
    if( (mask == 0) || (mask & 0x20000000) ) [g_spu_base + 0x1fa] = h(hu[A0 + 0x3e]);
    if( (mask == 0) || (mask & 0x40000000) ) [g_spu_base + 0x1fc] = h(hu[A0 + 0x40]);
    if( (mask == 0) || (mask & 0x80000000) ) [g_spu_base + 0x1fe] = h(hu[A0 + 0x42]);
}
...
s32 system_spu_reverb_clear_workarea( s32 mode )
{
    [SP + 0x10] = w(0);
    ...
    {
        size = (0x10000 - w[0x8004ab5c + mode * 4]) << g_spu_shift;
        dst = w[0x8004ab5c + mode * 4] << g_spu_shift;
    }

    // set to always transfer by dma
    u32 mode_s = g_transfer_mode2;
    bool mode_restore = false;
    if( mode_s == SPU_TRANSFER_BY_IO )
    {
        g_transfer_mode2 = SPU_TRANSFER_BY_DMA;
        mode_restore = true;
    }
    ...
    while( write )
    {
        ...
        func36a18( 0x2, dst ); // set address in spu to write to 0x1f801da6
        func36a18( 0x1 ); // set DMAwrite
        func36a18( 0x3, 0x8004a6ec, size_to_write );
    ...
    if( mode_restore )
    {
        g_transfer_mode2 = mode_s;
    }

    if( w[SP + 0x10] != 0 )
    {
        g_transfer_callback = w[SP + 0x10];
    }
...
long system_psyq_spu_set_irq( long on_off )
{
    ...
    {
        [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & ffbf); // remove IRQ9 Enable (0=Disabled/Acknowledge, 1=Enabled; only when Bit15=1)

        V1 = 0;
        while( hu[g_spu_base + 0x1aa] & 0x0040 )
        {
            V1 += 1;
            if( V1 >= 0xf01 )
            {
    ...
    {
        [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0040); // add IRQ9 Enable (0=Disabled/Acknowledge, 1=Enabled; only when Bit15=1)

        V1 = 0;
        while( ( hu[g_spu_base + 0x1aa] & 0x0040 ) == 0 )
        {
            V1 += 1;
            if( V1 >= 0xf01 )
            {
...
{
    S0 = g_spu_irq_callback;
    if( func != S0 )
    {
        g_spu_irq_callback = func;
        func38c48( func );
    }
    return S0;
}
...
void system_psyq_spu_set_key( long on_off, u_long voice_bit )
{
    if( on_off == SPU_OFF )
    {
        ...
        {
            [g_spu_base + 0x18c] = w(voice_bit); // Voice 0..23 Key OFF (Start Release) (W)
            g_spu_key &= ~voice_bit;
        }
    }
    else if( on_off == SPU_ON )
    {
        ...
        {
            [g_spu_base + 0x188] = w(voice_bit); // Voice 0..23 Key ON (Start Attack/Decay/Sustain) (W)
            g_spu_key |= voice_bit;
        }
    }
}
...
long system_psyq_spu_get_key_status( u_long voice_bit )
{
    ...
    adsr_vol = hu[g_spu_base + A1 * 0x10 + 0xc]; // Voice 0..23 Current ADSR volume
...
{
    V0 = g_transfer_callback;
    if( func != V0 ) g_transfer_callback = func;
    return V0;
}
...
void system_psyq_spu_set_common_attr( SpuCommonAttr* attr )
{
    ...
    if( (mask == 0) || (mask & SPU_COMMON_MVOLL) )
    {
        ...
        [g_spu_base + 0x180] = h((A2 & 0x7fff) | A1); // Main Volume Left
    }
    ...
    if( (mask == 0) || (mask & SPU_COMMON_MVOLR) )
    {
        ...
        [g_spu_base + 0x182] = h((T0 & 0x7fff) | A1); // Main Volume Right
    }

    if( (mask == 0) || (mask & SPU_COMMON_CDVOLL) ) [g_spu_base + 0x1b0] = h(attr->cd.volume.left); // CD Volume Left
    if( (mask == 0) || (mask & SPU_COMMON_CDVOLR) ) [g_spu_base + 0x1b2] = h(attr->cd.volume.right); // CD Volume Right
    if( (mask == 0) || (mask & SPU_COMMON_EXTVOLL) ) [g_spu_base + 0x1b4] = h(attr->ext.volume.left); // Extern Volume Left
    if( (mask == 0) || (mask & SPU_COMMON_EXTVOLR) ) [g_spu_base + 0x1b6] = h(attr->ext.volume.right); // Extern Volume Right

    if( (mask == 0) || (mask & SPU_COMMON_CDREV) )
    {
        if( attr->cd.reverb == 0 )
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xfffb);
        }
        else
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0004); // CD Audio Reverb
        }
    }
    ...
    if( (mask == 0) || (mask & SPU_COMMON_CDMIX) )
    {
        if( attr->cd.mix == 0 )
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xfffe);
        }
        else
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0001); // CD Audio Enable
        }
    }
    ...
    if( (mask == 0) || (mask & SPU_COMMON_EXTREV) )
    {
        ...
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xfff7);
        }
        else
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0008); // External Audio Reverb
        }
    }
    ...
    if( (mask == 0) || (mask & SPU_COMMON_EXTMIX) )
    {
        if( attr->ext.mix == 0 )
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] & 0xfffd);
        }
        else
        {
            [g_spu_base + 0x1aa] = h(hu[g_spu_base + 0x1aa] | 0x0002); // External Audio Enable
        }
    }
}
...
{
    [g_spu_base + voiceNum * 0x10 + 0x0] = h(volumeL & 0x7fff);
    [g_spu_base + voiceNum * 0x10 + 0x2] = h(volumeR & 0x7fff);
...
void system_psyq_spu_set_voice_volume_attr( int voiceNum, short volumeL, short volumeR, short volModeL, short volModeR )
{
    ...
    [g_spu_base + voiceNum * 0x10 + 0x0] = h(T0 | (volumeL & 0x7fff));
    ...
    [g_spu_base + voiceNum * 0x10 + 0x2] = h(T1 | (volumeR & 0x7fff));
...
{
    [g_spu_base + voiceNum * 0x10 + 0x4] = h(pitch);
...
{
    [g_spu_base + voiceNum * 0x10 + 0x8] = h((hu[g_spu_base + voiceNum * 0x10 + 0x8] & 0xff0f) | (DR << 0x4));
...
{
    [g_spu_base + voiceNum * 0x10 + 0x8] = h((hu[g_spu_base + voiceNum * 0x10 + 0x8] & 0xfff0) | SL); // sustain level
...
{
    [g_spu_base + voiceNum * 0x10 + 0x8] = h((hu[g_spu_base + voiceNum * 0x10 + 0x8] & 0x00ff) | ((AR | ((Armode == 0x5) << 0x7)) << 0x8));
...
void system_psyq_spu_set_voice_sr_attr( int voiceNum, u_short SR, long SRmode )
{
    ...
    [g_spu_base + voiceNum * 0x10 + 0xa] = h((hu[g_spu_base + voiceNum * 0x10 + 0xa] & 0x003f) | ((SR | A3) << 0x6));
...
{
    [g_spu_base + voiceNum * 0x10 + 0xa] = h((hu[g_spu_base + voiceNum * 0x10 + 0xa] & 0xffc0) | ((RRmode == 0x7) << 0x5) | A1);
