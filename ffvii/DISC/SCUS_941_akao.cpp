...
void func2a094( A0, A1, A2, A3 )
{
    ...
    [channel + 0x108 + 0x0] = w(0x80049c40);
    [channel + 0x108 + 0x50] = w(-1);
    [channel + 0x108 + 0x54] = h(0x1);
    [channel + 0x108 + 0x56] = h(0x101);

    if( A2 != 0 )
    {
        S1 = 1;

        func29bac( channel, A2 );
    ...
    if( S4 != 0 )
    {
        ...
        func29bac( channel + 0x108, S4 );

        [channel + 0x108 + 0x60] = h(S3);
        [channel + 0x108 + 0x62] = h(0);
    }
...
void func2a28c( u16 offset1, u32 offset2 )
{
    ...
    [channel1 + 0x54] = h(2);
    [channel2 + 0x54] = h(2);
    channel1->length_1 = 0x1;
    channel1->length_2 = 0x0;
    channel2->length_1 = 0x1;
    channel2->length_2 = 0x0;
...
void func2a43c()
{
    ...
    if( hu[0x80099e0c] == 0x2 )
    {
        ...
        [0x80099fd8] = w(w[0x80099fd8] & w[0x80099fcc] & 0xff3fffff);
    }
    else
    {
        [0x80099fd0] = w(0);
        [0x80099fd4] = w(0);
        [0x80099fd8] = w(w[0x80099fcc]);
    }
}



void func2a510( u16 channel_id, u16 pair_id )
{
    A2 = 0x80099890 + channel_id * 0x108;
    A3 = pair_id * 2;
    A1 = A1 & ffff;
        ...
        while( channels_mask != 0 )
        {
            if( channels_mask & channel_mask )
            {
                if( func31a70( channel ) == 0xca ) // loop return opcode
                {
                    ...
                    [channel + 0xe0] = w(0x0001ff93);
                }
                else
                {
                    if( hu[channel + 0x54] != 0x2 )
                    {
                        [0x80099fcc] = w(w[0x80099fcc] & ~channel_mask);
                        [0x80099fd0] = w(w[0x80099fd0] & ~channel_mask);
                        [0x80099fd4] = w(w[0x80099fd4] & ~channel_mask);
                    }
                }
...
void func2aabc( u8 save_id )
{
    ...
    [0x8009a104 + 0x4a] = h(hu[0x8008337e + V1 + 0000]);
    ...
    while( channels_mask != 0 )
    {
        if( channels_mask & channel_mask )
        {
            [channel + 0x56] = h(hu[channel + 0x56] + 0x202);
            [channel + 0xe0] = w(w[channel + 0xe0] | 0x0001ff93);
    ...
    if( (h[0x80062f44] == 0) && (hu[0x8009c5a8] == 0) )
    {
...
void func2afb8()
{
    ...
    for( int i = 0x18; i != 0; --i )
    {
        ...
        [channel + 0x24] = w(w[channel + 0x24] + 0x18);
        channel += 0x108;
    }
    ...
    for( int i = 0; i < 0x18; ++i )
    {
        [0x8009c5a0 + i * 0xc] = w(0x007f8000);
        [0x8009c5a8 + i * 0xc] = h(A2 * T0);
        [0x8009c5a4 + i * 0xc] = w(0 - (0x007f8000 / (T0 * A2)));
        A2 -= 1;
    }
...
void func2e478( ChannelData* channel, AkaoConfig* config, u32 mask )
{
    ...
    if( hu[channel + 0x62] != 0 )
    {
        ...
        A1 = hu[channel + 0x60] + h[channel + 0xca];

        if( (A1 & 0xff00) != (hu[channel + 0x60] & 0xff00) )
        {
            [channel + 0xe0] = w(w[channel + 0xe0] | 0x00000003);
        }
    ...
    if( hu[channel + 0x92] != 0 )
    {
        ...
        if (hu[channel + 0x88] == 0)
        {
            if( hu[channel + 0x8c] != 1 )
            {
                ...
                A1 = ((((((h[channel + 0x46] * w[channel + 0x2c]) >> 0x7) * (hu[channel + 0x90] >> 0x8)) << 0x9) >> 0x10) * h[A0]) >> 0xf;
...
{
    volume_level = (h[channel + 0x46] * w[channel + 0x2c]) >> 0x7;
    ...
    if( w[channel + 0x38] & 0x00000020 )
    {
        ...
        volume_level = (((hu[channel - 0x108 + 0xf8] << 0x11) >> 0x10) * w[channel + 0x2c]) >> 0x7;
    }
    ...
    if( channel->attr.mask & (SPU_VOICE_VOLL | SPU_VOICE_VOLR) )
    {
        ...
        if( hu[0x8009c5a8] != 0 )
        {
            volume_level = (volume_level * h[0x8009c5a2 + channel_id * 0xc]) >> 0x7;
        }

        volume_pan = (h[channel + 0xda] + hu[channel + 0x60] >> 0x8) & 0xff;

        if( w[0x8009a104 + 0x0] == 1 ) // stereo
        {
            channel->attr.vol_l = (volume_level * h[0x80049c44 + volume_pan * 0x2]) >> 0xf;
            channel->attr.vol_r = (volume_level * h[0x80049e44 + volume_pan * 0x2]) >> 0xf;
        }
...
{
    T0 = (h[channel + 0x46] * w[channel + 0x2c]) >> 0x7;
    ...
    {
        channel->attr.mask |= SPU_VOICE_VOLL | SPU_VOICE_VOLR;
        T0 = (((hu[channel - 0x10] << 0x11) >> 0x10) * w[channel + 0x2c]) >> 0x7;
    }
    ...
    if( channel->attr.mask & (SPU_VOICE_VOLL | SPU_VOICE_VOLR) )
    {
        ...
        if( hu[channel + 0x54] != 0x2 )
        {
            A0 = ((hu[channel + 0x60] >> 0x8) + h[channel + 0xda]) & 0xff;
            T0 = (T0 * (h[channel + 0xc6] >> 0x8)) >> 0x7;
        }

        if( w[0x8009a104 + 0x0] == 0x1 )
        {
            [channel + 0x104] = h((T0 * h[0x80049c44 + A0 * 0x2]) >> 0xf);
            [channel + 0x106] = h((T0 * h[0x80049e44 + A0 * 0x2]) >> 0xf);
        }
        else if( w[0x8009a104 + 0x0] == 0x4 )
        {
            ...
            {
                [channel + 0x106] = h(~hu[channel + 0x106]);
            }
            else
            {
                [channel + 0x104] = h(~hu[channel + 0x104]);
            }
        }
        else
        {
            [channel + 0x104] = h((T0 * h[0x80049cc4]) >> 0xf);
            [channel + 0x106] = h((T0 * h[0x80049cc4]) >> 0xf);
        }
    }
    ...
    {
        if( (hu[channel + 0x54] != 0x2) && (bu[channel + 0x3d] != 0) )
        {
...
void system_akao_update_dynamic()
{
    ...
    if( w[0x8009a104 + 0x38] & 0x00000010 )
    {
        ...
        {
            system_sound_spu_set_noise_frequency( hu[0x8009a104 + 0x52] );
        }
        ...
        while( channels_mask != 0 )
        {
            if( channels_mask & channel_mask )
            {
                ...
                if( w[channel + e0] != 0 )
                {
                    ...
                    if( A1 & 0x00000100 )
                    {
                        system_akao_update_volume_to_self_and_mirror( channel, S5, w[channel + 0x24] - 0x18 );
                    }
                    ...
                    else
                    {
                        ...
                        if( w[channel + 0x38] & 0x00000400 )
                        {
                            ...
                            if( S5 & ( 1 << A0 ) )
                            {
                                system_akao_update_params_to_spu( A0, channel + 0xdc );

                                if( updated_mask & channel_mask )
                                {
                                    updated_mask = (updated_mask | (1 << w[channel + 28])) & (0 NOR channel_mask);
                                }
                            }
                        }
                        else
    ...
    if( w[0x80099fcc] != 0 )
    {
        ...
        channels_mask = w[0x80099fcc] & w[0x80099fd4];
...
void func2fe48()()
{
    ...
    if( w[0x8009a164 + 0x4] != 0 )
    {
        ...
        channels_mask = w[0x8009a164 + 0x10] & w[0x80062f68] & A3;
...
void func2ff4c()
{
    ...
    A3 = ~(w[0x80099fcc] | w[0x80062f00]);
    channels_mask = A3 & (w[0x8009a164 + 0x2c] & w[0x80062f68]);
    ...
    A3 = ~(w[0x80062f68] | w[0x80099fcc] | w[0x80062f00]);
    channels_mask = A3 & w[0x8009a104 + 0x2c];
...
void func30038()
{
    ...
    A3 = ~(w[0x80099fcc] | w[0x80062f00]);
    channels_mask = w[0x8009a164 + 0x30] & w[0x80062f68] & A3;
    ...
    {
        A3 = ~(w[0x80062f68] | w[0x80099fcc] | w[0x80062f00]);
        channels_mask = A3 & w[0x8009a104 + 0x30];
...
void func30148()
{
    ...
    A3 = ~(w[0x80099fcc] | w[0x80062f00]);
    channels_mask = (w[0x8009a164 + 0x34] & w[0x80062f68]) & A3;
    ...
    A3 = ~(w[0x80062f68] | w[0x80099fcc] | w[0x80062f00]);
    channels_mask = A3 & w[0x8009a104 + 0x34];
...
void system_akao_execute_sequence( ChannelData* channel, AkaoConfig* config, u32 mask )
{
    ...
    if( opcode != 0xa0 )
    {
        ...
        if( h[channel + 0xc4] != 0 )
        {
            channel->length_1 = h[channel + 0xc4];
            channel->length_2 = h[channel + 0xc4];
        }
        ...
        [channel + 0xc2] = h(channel->length_1);
        ...
        else if( opcode < 0x84 )
        {
            ...
            [channel + 0xe0] = w(w[channel + 0xe0] | 0x00000013);
