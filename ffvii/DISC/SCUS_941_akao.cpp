...
void func29a50()
{
    ...
    {
        g_channels_1[0x10].attr.mask = SPU_VOICE_VOLL |
                                       SPU_VOICE_VOLR |
                                       SPU_VOICE_PITCH |
                                       SPU_VOICE_WDSA |
    ...
    {
        g_channels_1[0x11].attr.mask = SPU_VOICE_VOLL |
                                       SPU_VOICE_VOLR |
                                       SPU_VOICE_PITCH |
                                       SPU_VOICE_WDSA |
                                       SPU_VOICE_ADSR_AMODE |
...
void func2d1e4()
{
    g_akao_voice_attr.mask = 0x1ff93;
...
void system_akao_update_pitch_and_volume_lfo( ChannelData* channel, channel_mask, channel_id )
{
    ...
    // update vibrato
    if( (w[channel + 0x38] & 0x00000001) && (channel->vibrato_delay_cur == 0) )
    {
...
void system_akao_execute_sequence( ChannelData* channel, AkaoConfig* config, u32 mask )
{
    ...
    if( opcode != 0xa0 )
    {
        ...
        else if( opcode < 0x84 )
        {
            ...
            if( w[channel + 0x38] & 0x00000008 ) // drum mode on
            {
            ...
            if( w[channel + 0x38] & 0x00000001 ) // vibrato update
            {
