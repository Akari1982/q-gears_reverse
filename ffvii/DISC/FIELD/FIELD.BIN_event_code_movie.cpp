int field_event_opcode_f8_pmvie()
{
    ...
    {
        if (field_struct->cmd != FIELD_CMD_MOVIE_PLAY) return 1;
    ...
    field_struct->cmd = FIELD_CMD_MOVIE_PLAY;
    field_struct->arg = bu[events_data + script_cur + 0x1];
    [field_struct + 0x26] = h(0);
...
int field_event_opcode_f9_movie()
{
    ...
    if (bu[0x8009d820] & 0x3) field_debug_event_opcode("movie", 0);
    ...
    if (field_struct->cmd == FIELD_CMD_MOVIE_CONT)
    {
        if (h[field_struct + 0x26] == 0x1) return 1;
        if (h[field_struct + 0x26] != 0x2) return 1;
    ...
    if (field_struct->cmd >= FIELD_CMD_CREDITS)
    {
        if (field_struct->cmd != FIELD_CMD_MOVIE_STOP) return 1;
    ...
    if (field_struct->cmd == FIELD_CMD_NONE)
    {
        field_struct->cmd = FIELD_CMD_MOVIE_CONT;
        [field_struct + 0x26] = h(0);
    }
