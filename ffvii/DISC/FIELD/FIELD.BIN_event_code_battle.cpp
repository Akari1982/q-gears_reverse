...
int field_event_opcode_4b_btltb()
{
    ...
    if (bu[0x8009d820] & 0x3) field_debug_event_opcode("btltb", 0x2);
...
int field_event_opcode_70_batle()
{
    ...
    if (field_struct->cmd == FIELD_CMD_NONE)
    {
        ...
        field_struct->cmd = FIELD_CMD_BATTLE;

        V0 = ;

        [0x8007ebe0] = b(0x1);
        field_struct->arg = field_event_read_memory_s16(0x2, 0x2);
        [field_struct + 0x26] = h(0);
    }
    else if (field_struct->cmd == FIELD_CMD_BATTLE)
    {
