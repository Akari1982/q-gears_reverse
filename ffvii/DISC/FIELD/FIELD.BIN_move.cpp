...
int field_entity_move_line_check(FieldEntity* entity, u32 line_data, VECTOR* pos)
{
    s32 slip_disable = 0;
    ...
    for (int i = 0; i < 0x20; ++i) // go through all lines
    {
        if (bu[line_data + i * 0x18 + 0xc] == 0x1) // if line active
        {
            ...
            s32 sqr_dist = field_entity_move_sqr_dist_to_line(line_data + i * 0x18, &v_old, &proj);
            ...
            if ((sqr_dist != -0x1) && (sqr_dist < (entity->solid_range * entity->solid_range)))
            {
                ...
                {
                    [line_data + i * 0x18 + 0x14] = b(field_entity_move_dir_by_vec(&v_old, &proj, &sqr_dist));
...
s32 field_entity_move_sqr_dist_to_line(s32 line_offset, VECTOR* pos, VECTOR* ret)
{
...
u8 field_entity_move_dir_by_vec(VECTOR* current_pos, VECTOR* dest_pos, u32& dist)
{
...
void field_entity_move_gateway_check(FieldEntity* entity, u32 trigger_data, VECTOR* new_pos)
{
    VECTOR v_old;
    v_old.vx = entity->pos_x >> 0xc;
    v_old.vy = entity->pos_y >> 0xc;
    ...
    for (int i = 0; i < 0xc; ++i)
    {
        if (hu[S0 + 0xa] != 0x7fff)
        {
            u32 dist = field_entity_move_sqr_dist_to_line(trigger_data + i * 0x18, &v_old, 0x1f800020);
            ...
            if ((dist != -1) && (dist < (hu[entity + 0x6c] * hu[entity + 0x6c]))
            {
                ...
                if (((side_new > 0) && (side_old <= 0)) || ((side_old > 0) && (side_new <= 0)) || ((side_new >= 0) && (side_old < 0)) || ((side_old >= 0) && (side_new < 0))
                {
                    field_entity_move_gateway_map_load(trigger_data + i * 0x18);
                }
            }
        }
    }
...
void field_entity_move_gateway_map_load(u32 trigger_data)
{
...
void field_entity_move_trigger_check(FieldEntity* entity, trigger_data)
{
    ...
    for (int i = 0; i < 0xc; ++i)
    {
        ...
        {
            u32 dist = field_entity_move_sqr_dist_to_line(trigger_data + i * 0x10, &vec1, &vec2);

            // if we closer to line than solid range
            if ((dist != -0x1) && (dist < (entity->solid_range * entity->solid_range)))
            {
                // if coords change
                if ((vec1.vx != vec2.vx) || (vec1.vy != vec2.vy))
                {
                    u8 dir = field_entity_move_dir_by_vec(&vec1, &vec2, &dist);
                    dir = (dir - entity->move_dir + 0x40) & 0xff;
                    if (dir >= 0x80) continue;
                }

                if (field_entity_move_bg_trigger_activate(trigger_data + i * 0x10, bu[trigger_data + i * 0x10 + 0xe]) == 0x1)
                {
            ...
            else
            {
                ...
                {
                    if (field_entity_move_bg_trigger_activate(trigger_data + i * 0x10, 0x1) == 0x1)
                    {
                        u8 sound = bu[trigger_data + i * 0x10 + 0xf];
                        func1117c(sound_id[sound]); // play sound
                    }
                ...
                if (state != 0x5)
                {
                    if (field_entity_move_bg_trigger_activate(trigger_data + i * 0x10, 0) == 0x1)
                    {
                        u8 sound = bu[trigger_data + i * 0x10 + 0xf];
                        func1117c(sound_id[sound]); // play sound
                    }
...
s16 field_entity_move_bg_trigger_activate(trigger_data, state)
{
    s16 ret = 0;
...
void field_entity_move_bg_trigger_init(u32 trigger_data)
{
    for (int i = 0; i < 0xc; ++i)
    {
        if (bu[trigger_data + i * 0x10 + 0xc] != 0xff) // trigger exist
        {
            ...
            {
                field_entity_move_bg_trigger_activate(trigger_data + i * 0x10, 0x1);
            }
            else if ((state == 0x1) || (state == 0x3) || (state == 0x5))
            {
                field_entity_move_bg_trigger_activate(trigger_data + i * 0x10, 0);
            }
        }
    }
}
...
void field_line_clear_all_actors(s32 lines)
{
